...
pathwayOS Connect is disabled in a fresh clone. To enable it for source builds, add a repository-root `.env`
or `.env.local` file:
...
deployment defaults.
`vp run --filter pathwayos-relay deploy` invokes Alchemy from the relay directory, so Alchemy loads
...
The `pathwayos connect` commands authorize a headless environment with a separate Clerk OAuth application.
This uses an OAuth public client with PKCE, so the CLI stores no client secret.

In **Clerk Dashboard > OAuth applications**:

1. Create an OAuth application for the pathwayOS CLI.
...
5. Set `PATHWAYOS_CLERK_CLI_OAUTH_CLIENT_ID` in the repository-root `.env` file and release build
...
```sh
pathwayos connect login
pathwayos connect link
pathwayos connect status
pathwayos connect unlink
pathwayos connect logout
pathwayos serve
```

`pathwayos connect login` opens the Clerk authorization flow and stores the CLI credential without enabling
cloud exposure. `pathwayos connect link` installs the pinned managed `cloudflared` binary when needed,
authorizes when needed, and records durable intent to expose the environment. It works without a
running pathwayOS server. The next `pathwayos serve` or `pathwayos start` reconciles the relay link and launches the
managed tunnel. `pathwayos connect unlink` records disabled intent immediately, stops a reachable running
connector, and attempts to revoke the relay-side environment record. It retains the stored CLI
authorization so `pathwayos connect link` can re-enable exposure without another browser flow. `pathwayos connect
...
forward that port before running `pathwayos connect login` or `pathwayos connect link`:
...
| Setting | Value                          |
| ------- | ------------------------------ |
| Name    | `pathwayos-relay`              |
| Claims  | `{ "aud": "pathwayos-relay" }` |
...
Local desktop development uses `pathwayos-dev://app`, while packaged builds use `pathwayos://app`. Add the
matching origin to each Clerk instance's Backend API `allowed_origins` array as well. The development
Clerk instance should only need `pathwayos-dev://app`; the production Clerk instance should only need
`pathwayos://app`. `@clerk/electron` owns the native request adapter, encrypted Clerk token persistence,
...
curl -X PATCH https://api.clerk.com/v1/instance \
  ...
  -d '{"allowed_origins":["pathwayos://app"]}'
```
...
The production macOS bundle ID is `com.pathwayos.pathwayos`. To enable native passkeys:

1. Create an explicit macOS App ID for `com.pathwayos.pathwayos` in the Apple Developer portal and enable
   **Associated Domains**.
...
4. Confirm Clerk serves `https://<frontend-api>/.well-known/apple-app-site-association` and that
   `webcredentials.apps` contains `<TEAM_ID>.com.pathwayos.pathwayos`.
...
```dotenv
PATHWAYOS_APPLE_TEAM_ID=ABC1234567
PATHWAYOS_MACOS_PROVISIONING_PROFILE=/absolute/path/to/pathwayos.provisionprofile
# Optional: comma-separated override when Clerk's RP ID differs from the Frontend API hostname.
PATHWAYOS_CLERK_PASSKEY_RP_DOMAINS=example.clerk.accounts.dev,clerk.example.com
```

When `PATHWAYOS_CLERK_PASSKEY_RP_DOMAINS` is absent, the build derives the RP domain from
`PATHWAYOS_CLERK_PUBLISHABLE_KEY`. Signed macOS builds fail early if the Team ID, provisioning profile,
...
installed app executable with `VITE_DEV_SERVER_URL` and `PATHWAYOS_PORT` set. Rebuild the signed app
...
For the default development ports, run `bun run dev:web` in one terminal and launch the installed
binary from another:

```sh
VITE_DEV_SERVER_URL=http://127.0.0.1:5733 \
PATHWAYOS_PORT=13773 \
  "/Applications/pathwayOS (Alpha).app/Contents/MacOS/pathwayOS (Alpha)"
```
...
```sh
codesign --verify --deep --strict "/Applications/pathwayOS (Alpha).app"
codesign -d --entitlements :- "/Applications/pathwayOS (Alpha).app"
```
