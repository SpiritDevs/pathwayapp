...
This guide is for people who want to use more than one Claude setup in pathwayOS.
...
In pathwayOS Settings, your Claude provider can stay like this:
...
An empty `Claude HOME path` means pathwayOS uses your normal home directory.
...
Then add another Claude provider in pathwayOS:
...
pathwayOS only offers Claude providers that use the same Claude home for an existing thread. A
...
multiple files under its home directory, so pathwayOS keeps separate Claude homes isolated instead of
...
Add or edit a Claude provider in pathwayOS Settings:
...
Mark `ANTHROPIC_AUTH_TOKEN` as sensitive. pathwayOS stores the value as a server secret and does not
...
pathwayOS does not need a special Claude Code Router provider. Treat the router as a Claude
environment.
...
2. Add or configure a Claude provider in pathwayOS.
