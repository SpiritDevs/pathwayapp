...
This guide is for people who want to use more than one Codex account in pathwayOS.
...
- both accounts can see the same PathwayOS/Codex sessions
...
In pathwayOS Settings, name it something obvious:
...
In pathwayOS Settings, add another Codex provider:
...
pathwayOS shows the authenticated email for providers that report one. Emails are blurred by default;
...
If you add a third Codex provider with a completely different `CODEX_HOME path`, pathwayOS treats it
