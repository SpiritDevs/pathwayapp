...
- Reads the shared production pathwayOS Connect relay URL and Clerk client configuration before packaging clients.
...
- Publishes the CLI package (`apps/server`, npm package `pathwayos`) with OIDC trusted publishing from the same workflow file:
...
## pathwayOS Connect relay deployment
...
```sh
vp run --filter pathwayos-relay deploy -- --stage "$USER" --env-file .env.local
```
...
- `PATHWAYOS_WEB_ROUTER_URL`: defaults to `https://app.pathwayos.codes`.
- `PATHWAYOS_WEB_LATEST_DOMAIN`: defaults to `latest.app.pathwayos.codes`.
- `PATHWAYOS_WEB_NIGHTLY_DOMAIN`: defaults to `nightly.app.pathwayos.codes`.

Required Vercel domains:

- `app.pathwayos.codes`: the router domain users open, updated by stable releases.
- `latest.app.pathwayos.codes`: channel alias updated by stable releases.
- `nightly.app.pathwayos.codes`: channel alias updated by nightly releases.

The router domain uses `apps/web/vercel.ts` routes. Users opt into a channel by
visiting `/__pathwayos/channel?channel=latest` or
`/__pathwayos/channel?channel=nightly`; the router stores the
`pathwayos_web_channel` cookie and rewrites future requests on `app.pathwayos.codes` to
...
through `/__pathwayos/channel` on the router domain so the user's channel cookie is
...
4. Run one stable release deployment, or manually alias the current stable
   deployment, so `app.pathwayos.codes` points at a deployment containing the router
...
- Publishes the CLI package (`apps/server`, npm package `pathwayos`) to the `nightly` npm dist-tag using the same nightly version.
...
- Repository slug source:
  - `PATHWAYOS_DESKTOP_UPDATE_REPOSITORY` (format `owner/repo`), if set.
  - otherwise `GITHUB_REPOSITORY` from GitHub Actions.
- Temporary private-repo auth workaround:
  - set `PATHWAYOS_DESKTOP_UPDATE_GITHUB_TOKEN` (or `GH_TOKEN`) in the desktop app runtime environment.
...
1. Confirm npm org/user owns package `pathwayos` (or rename package first if needed).
...
2. Create an explicit App ID for `com.pathwayos.pathwayos` and enable Associated Domains.
...
10. Complete the Clerk Native API and AASA setup in [pathwayOS Connect Clerk Setup](../cloud/pathwayos-connect-clerk.md#desktop-passkeys).
...
- macOS build unsigned when expected signed:
  ...
  - Confirm the provisioning profile belongs to `APPLE_TEAM_ID.com.pathwayos.pathwayos` and includes
    Associated Domains.
