...
pathwayOS has one server-side observability model:
...
Completed spans are written as NDJSON records to `serverTracePath` (by default, `~/.pathwayos/userdata/logs/server.trace.ndjson`).
...
```bash
export PATHWAYOS_OTLP_TRACES_URL=http://localhost:4318/v1/traces
export PATHWAYOS_OTLP_METRICS_URL=http://localhost:4318/v1/metrics
export PATHWAYOS_OTLP_SERVICE_NAME=pathwayos-local
```

Optional:

```bash
export PATHWAYOS_TRACE_MIN_LEVEL=Info
export PATHWAYOS_TRACE_TIMING_ENABLED=true
```
...
Launch the actual app executable from the same shell so the desktop app and embedded backend inherit `PATHWAYOS_OTLP_*`.

macOS app bundle example:

```bash
PATHWAYOS_OTLP_TRACES_URL=http://localhost:4318/v1/traces \
PATHWAYOS_OTLP_METRICS_URL=http://localhost:4318/v1/metrics \
PATHWAYOS_OTLP_SERVICE_NAME=pathwayos-desktop \
"/Applications/pathwayOS.app/Contents/MacOS/pathwayOS"
```
...
```bash
tail -f "$PATHWAYOS_HOME/userdata/logs/server.trace.ndjson"
```
...
jq -c 'select(.exit._tag != "Success") | {
  ...
  exit,
  attributes
}' "$PATHWAYOS_HOME/userdata/logs/server.trace.ndjson"
```
...
jq -r 'select(.traceId == "TRACE_ID_HERE") | [
  ...
  (.parentSpanId // "-"),
  .durationMs
] | @tsv' "$PATHWAYOS_HOME/userdata/logs/server.trace.ndjson"
```
...
- service name such as `pathwayos-local`, `pathwayos-dev`, or `pathwayos-desktop`
...
- `pathwayos_rpc_request_duration`
- `pathwayos_orchestration_command_duration`
- `pathwayos_orchestration_command_ack_duration`
- `pathwayos_provider_turn_duration`
- `pathwayos_git_command_duration`
- `pathwayos_db_query_duration`

Counters tell you volume and failure rate:

- `pathwayos_rpc_requests_total`
- `pathwayos_orchestration_commands_total`
- `pathwayos_provider_turns_total`
- `pathwayos_git_commands_total`
- `pathwayos_db_queries_total`
...
`pathwayos_orchestration_command_ack_duration` measures:
...
1. Check `pathwayos_orchestration_command_ack_duration` by `commandType`.
...
- `PATHWAYOS_OTLP_TRACES_URL` was not set
...
- `PATHWAYOS_TRACE_FILE`: override trace file path
- `PATHWAYOS_TRACE_MAX_BYTES`: per-file rotation size, default `10485760`
- `PATHWAYOS_TRACE_MAX_FILES`: rotated file count, default `10`
- `PATHWAYOS_TRACE_BATCH_WINDOW_MS`: flush window, default `200`
- `PATHWAYOS_TRACE_MIN_LEVEL`: minimum trace level, default `Info`
- `PATHWAYOS_TRACE_TIMING_ENABLED`: enable timing metadata, default `true`

OTLP export:

- `PATHWAYOS_OTLP_TRACES_URL`: OTLP trace endpoint
- `PATHWAYOS_OTLP_METRICS_URL`: OTLP metric endpoint
- `PATHWAYOS_OTLP_EXPORT_INTERVAL_MS`: export interval, default `10000`
- `PATHWAYOS_OTLP_SERVICE_NAME`: service name, default `pathwayos-server`
