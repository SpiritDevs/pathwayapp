...
- `pathwayos-relay-traces-prod`, an OpenTelemetry trace dataset for Worker requests
- `pathwayos-relay-otel-ingest-prod`, a dataset-scoped ingest token bound to the Worker
- `pathwayos-relay-recent-spans-prod`, a view of recent request and endpoint spans

Alchemy stages append their sanitized stage name to isolate resources, for example
`pathwayos-relay-traces-dev-julius` for a personal stage.
