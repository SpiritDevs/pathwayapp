...
- `/apps/desktop`: Electron shell. Spawns a desktop-scoped `pathwayos` backend process and loads the shared web app.
...
- `/packages/shared`: Shared runtime utilities consumed by both server and web. Uses explicit subpath exports (e.g. `@pathwayos/shared/git`, `@pathwayos/shared/DrainableWorker`) — no barrel index.
