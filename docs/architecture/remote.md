...
This document describes the target architecture for first-class remote environments in pathwayOS.
...
- Keep the pathwayOS server as the execution boundary.
...
PathwayOS already has a clean runtime boundary: the client talks to a pathwayOS server over HTTP/WebSocket, and the server owns orchestration, providers, terminals, git, and filesystem operations.
...
└───────────────┬──────────────────────────────┘
                │
                │ connects to one pathwayOS server
                │
┌───────────────▼──────────────────────────────┐
│ Execution environment = one pathwayOS server       │
│                                              │
...
The important decision is that remoteness is expressed at the environment connection layer, not by splitting the PathwayOS runtime itself.

## Domain model

### ExecutionEnvironment

An `ExecutionEnvironment` is one running pathwayOS server instance.
...
How does the client speak WebSocket to a pathwayOS server?
...
- best fit for hosted or self-managed remote PathwayOS deployments
...
For PathwayOS, tunnels are best modeled as another `AccessEndpoint`, not as a different kind of environment.
...
Tailscale-backed access sits here architecturally even though the current implementation is endpoint discovery rather than a PathwayOS-managed tunnel. It contributes private-network endpoints and lets the existing HTTP/WebSocket client path do the actual connection.
...
- reach a machine
- probe it
- launch or reuse a remote pathwayOS server
...
How does a pathwayOS server come to exist on the target machine?
...
The user or operator already runs PathwayOS on the target machine, and the client connects through a direct or tunneled WebSocket endpoint.
...
What should be different in PathwayOS:
...
The recommended PathwayOS flow is:

1. Desktop connects over SSH.
2. Desktop probes the remote machine and verifies PathwayOS availability.
3. Desktop launches or reuses a remote pathwayOS server.
...
This is the inverse of remote launch: a local pathwayOS server is already running, and the client publishes it through a tunnel.
...
- A manually hosted pathwayOS server might be reached through direct `wss`.
...
PathwayOS already supports a WebSocket auth token on the server. That should become a first-class part of environment access rather than remaining an incidental query parameter convention.
...
PathwayOS should not copy that part.

PathwayOS already has the right runtime boundary:

- one pathwayOS server per environment
- ordinary HTTP/WebSocket between client and environment

So PathwayOS should borrow Zed's launch discipline, not its transport protocol.
