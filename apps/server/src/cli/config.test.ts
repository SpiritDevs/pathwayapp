...
const makeDesktopBootstrap = (
  ...
  noBrowser: true,
  port: 4888,
  pathwayosHome: "/tmp/pathwayos-bootstrap-home",
  host: "127.0.0.1",
...
it.layer(NodeServices.layer)("cli config resolution", (it) => {
  ...
  it.effect("falls back to effect/config values when flags are omitted", () =>
    Effect.gen(function* () {
      ...
      const baseDir = join(NodeOS.tmpdir(), "pathwayos-cli-config-env-base");
          ...
          Layer.mergeAll(
            ConfigProvider.layer(
              ConfigProvider.fromEnv({
                env: {
                  PATHWAYOS_LOG_LEVEL: "Warn",
                  PATHWAYOS_MODE: "desktop",
                  PATHWAYOS_PORT: "4001",
                  PATHWAYOS_HOST: "0.0.0.0",
                  PATHWAYOS_HOME: baseDir,
                  VITE_DEV_SERVER_URL: "http://127.0.0.1:5173",
                  PATHWAYOS_NO_BROWSER: "true",
                  PATHWAYOS_AUTO_BOOTSTRAP_PROJECT_FROM_CWD: "false",
                  PATHWAYOS_LOG_WS_EVENTS: "true",
                },
              }),
            ),
            NetService.layer,
  ...
  it.effect("uses CLI flags when provided", () =>
    Effect.gen(function* () {
      const { join } = yield* Path.Path;
      const baseDir = join(NodeOS.tmpdir(), "pathwayos-cli-config-flags-base");
            ...
            ConfigProvider.layer(
              ConfigProvider.fromEnv({
                env: {
                  ...
                  PATHWAYOS_PORT: "4001",
                  PATHWAYOS_HOST: "0.0.0.0",
                  PATHWAYOS_HOME: join(NodeOS.tmpdir(), "ignored-base"),
                  VITE_DEV_SERVER_URL: "http://127.0.0.1:5173",
                  PATHWAYOS_NO_BROWSER: "false",
                  PATHWAYOS_AUTO_BOOTSTRAP_PROJECT_FROM_CWD: "false",
                  PATHWAYOS_LOG_WS_EVENTS: "false",
                },
              }),
            ),
            NetService.layer,
  ...
  it.effect("preserves explicit false CLI boolean flags over env and bootstrap values", () =>
    Effect.gen(function* () {
      const { join } = yield* Path.Path;
      const baseDir = join(NodeOS.tmpdir(), "pathwayos-cli-config-false-flags");
        ...
        Effect.provide(
          Layer.mergeAll(
            ConfigProvider.layer(
              ConfigProvider.fromEnv({
                env: {
                  PATHWAYOS_BOOTSTRAP_FD: String(fd),
                  PATHWAYOS_NO_BROWSER: "true",
                  PATHWAYOS_AUTO_BOOTSTRAP_PROJECT_FROM_CWD: "true",
                  PATHWAYOS_LOG_WS_EVENTS: "true",
                },
              }),
            ),
  ...
  it.effect("uses bootstrap envelope values as fallbacks when flags and env are absent", () =>
    Effect.gen(function* () {
      const { join } = yield* Path.Path;
      const baseDir = "/tmp/pathwayos-bootstrap-home";
      const fd = yield* openBootstrapFd(
        makeDesktopBootstrap({
          port: 4888,
          host: "127.0.0.2",
          pathwayosHome: baseDir,
          noBrowser: true,
  ...
  it.effect("creates derived runtime directories during config resolution", () =>
    Effect.gen(function* () {
      ...
      const baseDir = yield* fs.makeTempDirectoryScoped({ prefix: "pathwayos-cli-config-dirs-" });
  ...
  it.effect("applies flag then env precedence over bootstrap envelope values", () =>
    Effect.gen(function* () {
      const { join } = yield* Path.Path;
      const baseDir = join(NodeOS.tmpdir(), "pathwayos-cli-config-env-wins");
      const fd = yield* openBootstrapFd(
        makeDesktopBootstrap({
          port: 4888,
          host: "127.0.0.2",
  ...
  it.effect("falls back to persisted observability settings when env vars are absent", () =>
    Effect.gen(function* () {
      ...
      const baseDir = yield* fs.makeTempDirectoryScoped({
        prefix: "pathwayos-cli-config-settings-",
      });
      const derivedPaths = yield* deriveServerPaths(baseDir, undefined);
  ...
  it.effect("forces noBrowser and disables auto-bootstrap for headless startup presentation", () =>
    Effect.gen(function* () {
      const { join } = yield* Path.Path;
      const baseDir = join(NodeOS.tmpdir(), "pathwayos-cli-config-headless-base");
      const derivedPaths = yield* deriveServerPaths(baseDir, undefined);
