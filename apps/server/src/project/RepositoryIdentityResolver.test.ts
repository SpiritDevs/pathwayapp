...
it.layer(NodeServices.layer)("RepositoryIdentityResolverLive", (it) => {
  it.effect("normalizes equivalent GitHub remotes into a stable repository identity", () =>
    Effect.gen(function* () {
      ...
      const cwd = yield* fileSystem.makeTempDirectoryScoped({
        prefix: "pathwayos-repository-identity-test-",
      });

      yield* git(cwd, ["init"]);
      yield* git(cwd, ["remote", "add", "origin", "git@github.com:PathwayOS/pathwayos.git"]);
      ...
      expect(identity?.canonicalKey).toBe("github.com/pathwayos/pathwayos");
      expect(normalizeResolvedPath(resolvedIdentityRoot)).toBe(normalizeResolvedPath(resolvedCwd));
      expect(identity?.displayName).toBe("pathwayos/pathwayos");
      expect(identity?.provider).toBe("github");
      expect(identity?.owner).toBe("pathwayos");
      expect(identity?.name).toBe("pathwayos");
  ...
  it.effect("returns the git top-level root path when resolving from a nested workspace", () =>
    Effect.gen(function* () {
      ...
      const repoRoot = yield* fileSystem.makeTempDirectoryScoped({
        prefix: "pathwayos-repository-identity-nested-root-test-",
      });
      ...
      yield* git(repoRoot, ["remote", "add", "origin", "git@github.com:PathwayOS/pathwayos.git"]);
  ...
  it.effect("returns null for non-git folders and repos without remotes", () =>
    Effect.gen(function* () {
      ...
      const nonGitDir = yield* fileSystem.makeTempDirectoryScoped({
        prefix: "pathwayos-repository-identity-non-git-",
      });
      const gitDir = yield* fileSystem.makeTempDirectoryScoped({
        prefix: "pathwayos-repository-identity-no-remote-",
      });
  ...
  it.effect("prefers upstream over origin when both remotes are configured", () =>
    Effect.gen(function* () {
      const fileSystem = yield* FileSystem.FileSystem;
      const cwd = yield* fileSystem.makeTempDirectoryScoped({
        prefix: "pathwayos-repository-identity-upstream-test-",
      });

      yield* git(cwd, ["init"]);
      yield* git(cwd, ["remote", "add", "origin", "git@github.com:julius/pathwayos.git"]);
      yield* git(cwd, ["remote", "add", "upstream", "git@github.com:PathwayOS/pathwayos.git"]);
  ...
  it.effect("uses the last remote path segment as the repository name for nested groups", () =>
    Effect.gen(function* () {
      const fileSystem = yield* FileSystem.FileSystem;
      const cwd = yield* fileSystem.makeTempDirectoryScoped({
        prefix: "pathwayos-repository-identity-nested-group-test-",
      });
      ...
      yield* git(cwd, [
        ...
        "add",
        "origin",
        "git@gitlab.com:PathwayOS/platform/pathwayos.git",
      ]);
      ...
      expect(identity).not.toBeNull();
      expect(identity?.canonicalKey).toBe("gitlab.com/pathwayos/platform/pathwayos");
      expect(identity?.displayName).toBe("pathwayos/platform/pathwayos");
      expect(identity?.owner).toBe("pathwayos");
      expect(identity?.name).toBe("pathwayos");
    }).pipe(Effect.provide(RepositoryIdentityResolver.layer)),
  );
  ...
  it.effect(
    ...
    () =>
      Effect.gen(function* () {
        const fileSystem = yield* FileSystem.FileSystem;
        const cwd = yield* fileSystem.makeTempDirectoryScoped({
          prefix: "pathwayos-repository-identity-late-remote-test-",
        });
        ...
        expect(refreshedIdentity?.canonicalKey).toBe("github.com/pathwayos/pathwayos");
        expect(refreshedIdentity?.name).toBe("pathwayos");
      }).pipe(
        Effect.provide(
          Layer.merge(
            TestClock.layer(),
  ...
  it.effect("refreshes cached identities after the positive TTL when a remote changes", () =>
    Effect.gen(function* () {
      const fileSystem = yield* FileSystem.FileSystem;
      const cwd = yield* fileSystem.makeTempDirectoryScoped({
        prefix: "pathwayos-repository-identity-remote-change-test-",
      });
      ...
      expect(initialIdentity?.canonicalKey).toBe("github.com/pathwayos/pathwayos");

      yield* git(cwd, [
        "remote",
        "set-url",
        "origin",
        "git@github.com:PathwayOS/pathwayos-next.git",
      ]);

      const cachedIdentity = yield* resolver.resolve(cwd);
      expect(cachedIdentity).not.toBeNull();
      expect(cachedIdentity?.canonicalKey).toBe("github.com/pathwayos/pathwayos");

      yield* TestClock.adjust(Duration.millis(180));

      const refreshedIdentity = yield* resolver.resolve(cwd);
      expect(refreshedIdentity).not.toBeNull();
      expect(refreshedIdentity?.canonicalKey).toBe("github.com/pathwayos/pathwayos-next");
      expect(refreshedIdentity?.displayName).toBe("pathwayos/pathwayos-next");
      expect(refreshedIdentity?.name).toBe("pathwayos-next");
    }).pipe(
      Effect.provide(
        Layer.merge(
          TestClock.layer(),
