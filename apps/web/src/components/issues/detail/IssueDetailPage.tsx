...
import { CheckIcon, ChevronRightIcon, PencilIcon, RotateCcwIcon, SearchIcon, XIcon } from "lucide-react";
  ...
  return (
    <Empty className="h-full">
      <EmptyHeader>
        ...
        <EmptyDescription>No issue or identifier alias matches {props.identifier}.</EmptyDescription>
      </EmptyHeader>
      <EmptyContent>
        <Button render={<Link to="/issues" />} variant="outline">Search issues</Button>
      </EmptyContent>
    </Empty>
  );
}
...
export function IssueDetailPage(props: {
  ...
  if (!issue) {
    return <div className="flex h-full items-center justify-center text-sm text-muted-foreground">Loading issue…</div>;
  }
  ...
  return (
    <div className={cn("h-full min-h-0 bg-background", props.condensed ? "overflow-y-auto" : "overflow-hidden")}>
      {issue.deletedAt ? (
        <div className="flex items-center justify-between gap-3 border-b border-warning/30 bg-warning/8 px-5 py-2 text-sm">
          ...
          <Button onClick={() => void restoreIssue({ environmentId: props.issueRef.environmentId, input: { issueId: issue.id } })} size="xs" variant="outline">
            <RotateCcwIcon /> Restore
          </Button>
        </div>
      ) : null}
      <div className={cn("grid h-full min-h-0", props.condensed ? "grid-cols-1" : "grid-cols-[minmax(0,1fr)_18rem]")}>
        <main className={cn("min-w-0", props.condensed ? "px-5 py-5" : "overflow-y-auto px-8 py-6")}>
          <div className="mx-auto max-w-3xl">
            <div className="mb-5 flex items-center gap-1 text-xs text-muted-foreground">
              <button className="hover:text-foreground" onClick={() => void navigate({ to: "/issues" })} type="button">{team?.key ?? "Workspace"}</button>
              <ChevronRightIcon className="size-3" />
              <span>{issue.identifier}</span>
              {props.condensed && props.onClose ? (
                <Button aria-label="Close issue peek" className="ml-auto" onClick={props.onClose} size="icon-xs" variant="ghost"><XIcon /></Button>
              ) : null}
            </div>
              ...
              {editingTitle ? (
                <Input
                  ...
                  onKeyDown={(event) => {
                    ...
                    if (event.key === "Escape") { setTitle(issue.title); setEditingTitle(false); }
                  }}
                  size="lg"
                  value={title}
                />
              ) : (
                <button className="flex w-full items-start gap-2 text-left" onClick={() => setEditingTitle(true)} type="button">
                  <h1 className="min-w-0 flex-1 text-2xl font-semibold leading-tight tracking-tight">{issue.title}</h1>
            ...
            <section className="mb-5">
              <div className="mb-2 flex items-center justify-between">
                ...
                {!editingDescription ? (
                  <Button onClick={() => setEditingDescription(true)} size="xs" variant="ghost"><PencilIcon /> Edit</Button>
                ) : null}
              </div>
              {editingDescription ? (
                <div className="space-y-2">
                  <MarkdownEditor environmentId={props.issueRef.environmentId} minHeight={160} onChange={setDescription} value={description} />
                  <div className="flex justify-end gap-2">
                    <Button onClick={() => { setDescription(detail?.descriptionMd ?? ""); setEditingDescription(false); }} size="sm" variant="ghost">Cancel</Button>
                    <Button onClick={saveDescription} size="sm"><CheckIcon /> Save</Button>
                  </div>
                </div>
              ) : (
                <button className="min-h-16 w-full rounded-md py-1 text-left" onClick={() => setEditingDescription(true)} type="button">
            ...
            <section className="border-t pt-4">
              <h2 className="mb-1 text-xs font-medium uppercase tracking-wide text-muted-foreground">Discussion</h2>
              <CommentThread actors={actors} comments={detail?.comments ?? []} issueRef={props.issueRef} reactions={detail?.reactions ?? []} />
