...
const selectClassName =
  "h-7 min-w-0 rounded-md border border-transparent bg-transparent px-1 text-sm outline-none hover:border-input focus:border-ring";
    ...
    <aside className="min-w-0 border-l px-4 py-5">
      ...
      <PropertyRow label="State">
        <select
          ...
          onChange={(event) =>
            void update({ stateId: event.target.value as EnvironmentIssue["stateId"] })
          }
          value={props.issue.stateId}
        >
          ...
          onChange={(event) =>
            void update({
              assigneeActorId: event.target.value
                ? (event.target.value as EnvironmentIssue["assigneeActorId"])
                : null,
            })
          }
          value={selectValue(props.issue.assigneeActorId)}
        >
      ...
      <PropertyRow label="Priority">
        <select
          className={selectClassName}
          onChange={(event) =>
            void update({ priority: Number(event.target.value) as IssuePriority })
          }
      ...
      <PropertyRow label="Labels">
        <Popover>
          <PopoverTrigger
            render={
              <button
                className="min-h-7 rounded-md px-1 text-left text-sm hover:bg-accent"
                type="button"
              />
            }
          >
            {props.issue.labelIds.length === 0 ? (
              <span className="text-muted-foreground">No labels</span>
            ) : (
              `${props.issue.labelIds.length} selected`
            )}
          </PopoverTrigger>
                ...
                >
                  <input
                    ...
                    onChange={() =>
                      void update({
                        labelIds: checked
                          ? props.issue.labelIds.filter((id) => id !== label.id)
                          : [...props.issue.labelIds, label.id],
                      })
                    }
                    type="checkbox"
                  />
      ...
      {estimates.length > 0 ? (
        <PropertyRow label="Estimate">
          <select
            className={selectClassName}
            onChange={(event) =>
              void update({ estimate: event.target.value ? Number(event.target.value) : null })
            }
            value={props.issue.estimate ?? ""}
          >
            <option value="">No estimate</option>
            {estimates.map((estimate, index) => (
              <option key={estimate} value={estimate}>
                {team?.estimateScale === "tshirt" ? ["XS", "S", "M", "L", "XL"][index] : estimate}
              </option>
            ))}
          </select>
        </PropertyRow>
      ...
      <PropertyRow label="Due date">
        <Input
          nativeInput
          onChange={(event) => void update({ dueDate: event.target.value || null })}
          size="sm"
          type="date"
          ...
          onChange={(event) =>
            void update({
              cycleId: event.target.value
                ? (event.target.value as EnvironmentIssue["cycleId"])
                : null,
            })
          }
          value={selectValue(props.issue.cycleId)}
        >
          ...
          onChange={(event) =>
            void update({
              epicId: event.target.value
                ? (event.target.value as EnvironmentIssue["epicId"])
                : null,
              milestoneId: null,
            })
          }
          value={selectValue(props.issue.epicId)}
        >
          <option value="">No epic</option>
          {epics
            .filter((epic) => epic.deletedAt === null)
            .map((epic) => (
      ...
      <PropertyRow label="Milestone">
        <select
          className={selectClassName}
          disabled={!props.issue.epicId}
          onChange={(event) =>
            void update({
              milestoneId: event.target.value
                ? (event.target.value as EnvironmentIssue["milestoneId"])
                : null,
            })
          }
          value={selectValue(props.issue.milestoneId)}
        >
      ...
      <PropertyRow label="Team">
        <select
          className={selectClassName}
          onChange={(event) =>
            setPendingTeamId(event.target.value ? (event.target.value as IssueTeamId) : null)
          }
          value={selectValue(props.issue.teamId)}
        >
      ...
      <AlertDialog
        open={pendingTeamId !== undefined}
        onOpenChange={(open) => {
          if (!open) setPendingTeamId(undefined);
        }}
      >
        <AlertDialogPopup>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              The issue identifier will change to use the destination team key. Existing links will
              continue through its identifier alias.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
            ...
            <AlertDialogClose
              render={<Button />}
              onClick={() => {
                if (pendingTeamId !== undefined)
                  void moveTeam({
                    environmentId: props.issueRef.environmentId,
                    input: { issueId: props.issue.id, teamId: pendingTeamId },
                  });
