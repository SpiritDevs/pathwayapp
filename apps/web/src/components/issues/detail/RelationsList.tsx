...
function relatedIssueId(relation: IssueRelation, issueId: EnvironmentIssue["id"]): EnvironmentIssue["id"] {
...
export function RelationsList(props: {
    ...
    return issues
      .filter(
        (issue) =>
          ...
          (!normalized || `${issue.identifier} ${issue.title}`.toLocaleLowerCase().includes(normalized)),
      )
      .slice(0, 8);
  ...
  const categorized = (kind: RelationSection) =>
    props.relations.filter((relation) => {
      if (kind === "blocked-by") return relation.relationType === "blocks" && relation.relatedIssueId === props.issue.id;
      if (kind === "blocks") return relation.relationType === "blocks" && relation.issueId === props.issue.id;
  ...
  return (
    <section className="border-t py-4">
      <div className="mb-1 flex items-center justify-between">
        <h2 className="text-xs font-medium uppercase tracking-wide text-muted-foreground">Relations</h2>
        <Popover>
          ...
          <PopoverPopup align="end" className="w-80" viewportClassName="space-y-2 p-2">
            <div className="grid grid-cols-4 gap-1">
              {(["blocks", "blocked-by", "related", "duplicate"] as const).map((kind) => (
                <button className={`rounded px-1 py-1 text-[11px] ${section === kind ? "bg-accent text-foreground" : "text-muted-foreground"}`} key={kind} onClick={() => setSection(kind)} type="button">
                  {kind}
                </button>
              ))}
            </div>
            <Input onChange={(event) => setQuery(event.target.value)} placeholder="Search issues…" type="search" value={query} />
            <div className="max-h-56 overflow-y-auto">
              {candidates.map((candidate) => (
                <button className="flex w-full items-center gap-2 rounded px-2 py-1.5 text-left text-sm hover:bg-accent" key={candidate.id} onClick={() => void add(candidate)} type="button">
                  <span className="shrink-0 text-xs text-muted-foreground">{candidate.identifier}</span>
        ...
        return (
          <div className="mt-2" key={kind}>
            ...
            {entries.map((relation) => {
              const related = issues.find((issue) => issue.id === relatedIssueId(relation, props.issue.id));
              return (
                <div className="group flex items-center gap-2 py-1 text-sm" key={relation.id}>
                  ...
                  <span className="text-xs text-muted-foreground">{related?.identifier ?? "Issue"}</span>
                  <span className="min-w-0 flex-1 truncate">{related?.title ?? "Unavailable issue"}</span>
                  <Button aria-label="Remove relation" className="opacity-0 group-hover:opacity-100" onClick={() => void deleteRelation({ environmentId: props.issueRef.environmentId, input: { relationId: relation.id } })} size="icon-xs" variant="ghost">
                    <Trash2Icon />
                  </Button>
                </div>
              );
      ...
      {props.relations.length === 0 ? <p className="mt-2 text-sm text-muted-foreground">No relations.</p> : null}
    </section>
  );
}
