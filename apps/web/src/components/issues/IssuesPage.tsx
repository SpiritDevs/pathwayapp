...
export function IssuesPage() {
  ...
  const activeSavedViews = useMemo(
    () => savedViews.filter((item) => item.deletedAt === null),
    [savedViews],
  );
  const selectedView = activeSavedViews.find((view) => view.id === selectedViewId) ?? null;
  const visibleIssues = useMemo(
    () => filterIssues(allIssues, filters, display, activeStates),
    [activeStates, allIssues, display, filters],
  );
  const lookup = useMemo(
    ...
    [activeActors, activeCycles, activeEpics, activeLabels, activeStates, activeTeams],
  );
  const untriagedCount = allIssues.filter(
    (issue) => !issue.triaged && issue.deletedAt === null,
  ).length;
    ...
    <SidebarInset className="h-dvh min-h-0 overflow-hidden bg-background text-foreground">
      ...
      <div className="flex min-h-0 flex-1 flex-col">
        <header className="flex h-12 shrink-0 items-center gap-3 border-b px-4">
          ...
          <a href="/issues/triage" className="inline-flex">
            <Badge variant={untriagedCount ? "warning" : "outline"}>
              <InboxIcon /> Triage {untriagedCount}
            </Badge>
          </a>
          <DisplayOptions display={display} onChange={setDisplay} />
          <Button size="xs" onClick={openIssueQuickCreate} disabled={!meta.online}>
            <PlusIcon /> New issue
          </Button>
        </header>
        {!meta.online && meta.workspaceKey !== null ? (
          <Alert variant="warning" className="m-3 mb-0 rounded-lg py-2">
            <TriangleAlertIcon />
            <AlertDescription>Issues are read-only while offline</AlertDescription>
          </Alert>
        ) : null}
        <FilterBar
          filters={filters}
        ...
        {meta.workspaceKey === null ? (
          ...
          <Empty>
            ...
            <EmptyHeader>
              ...
              <EmptyDescription>
                Create a team to give issues a workflow, key, and ownership boundary.
              </EmptyDescription>
            </EmptyHeader>
            <EmptyContent>
              <Button render={<a href="/settings/teams" />}>Set up teams</Button>
            </EmptyContent>
          </Empty>
        ) : allIssues.length === 0 ? (
          <Empty>
            ...
            <EmptyHeader>
              <EmptyTitle>Your issue workspace is clear</EmptyTitle>
              <EmptyDescription>
                Create the first issue or save a view for the work you want to track.
              </EmptyDescription>
            </EmptyHeader>
            <EmptyContent>
              <Button onClick={openIssueQuickCreate} disabled={!meta.online}>
