...
import { CheckIcon, InboxIcon, BotIcon, XIcon } from "lucide-react";
...
export function TriageInbox() {
  ...
  const issues = useIssues().filter(
    (issue) => issue.environmentId === environmentId && issue.deletedAt === null,
  );
  ...
  const triageStateIds = new Set(
    states.filter((state) => state.category === "triage").map((state) => state.id),
  );
  ...
  const grouped = useMemo(() => {
    ...
    for (const issue of triageIssues) {
      ...
      else byTeam.set(issue.teamId, [issue]);
    }
  ...
  const destination = (
    ...
    category: "backlog" | "canceled",
  ): IssueStateId | null =>
    states
      .filter((state) => state.teamId === issue.teamId && state.category === category)
      .sort((a, b) => a.position - b.position)[0]?.id ?? null;
  const triage = async (
    targetIssues: ReadonlyArray<EnvironmentIssue>,
    ...
    try {
      for (const issue of targetIssues) {
        ...
        if (stateId === null) {
          setError(
            `${issue.identifier} has no ${action === "accept" ? "backlog" : "canceled"} state in its team.`,
          );
          continue;
        }
        await execute(() =>
          updateIssue({
            environmentId,
            input: { issueId: issue.id, patch: { stateId, triaged: true } },
          }),
        );
      }
      setSelected(new Set());
  ...
  return (
    <div className="flex min-h-0 flex-1 flex-col overflow-hidden bg-background">
      <header className="flex min-h-14 items-center justify-between gap-3 border-b px-5">
        <div>
          <h1 className="font-heading text-base font-semibold">Triage inbox</h1>
          <p className="text-xs text-muted-foreground">
            Review newly submitted issues before they enter a team backlog.
          </p>
        </div>
        {triageIssues.length > 0 ? (
          <div className="flex items-center gap-2">
            <Button
              size="sm"
              variant="outline"
              disabled={selectedIssues.length === 0 || pending.size > 0}
              onClick={() => void triage(selectedIssues, "decline")}
            >
              <XIcon />
              Decline {selectedIssues.length || ""}
            </Button>
            <Button
              size="sm"
              disabled={selectedIssues.length === 0 || pending.size > 0}
              onClick={() => void triage(selectedIssues, "accept")}
            >
              <CheckIcon />
              Accept {selectedIssues.length || ""}
            </Button>
          </div>
        ) : null}
      </header>
      ...
      {triageIssues.length === 0 ? (
        <Empty>
          <EmptyHeader>
            ...
            <EmptyDescription>There are no issues waiting for triage.</EmptyDescription>
          </EmptyHeader>
        </Empty>
      ) : (
        <div className="min-h-0 flex-1 overflow-y-auto p-5">
          <div className="mx-auto max-w-5xl space-y-6">
            {grouped.map(([teamId, teamIssues]) => {
              const team = teams.find((candidate) => candidate.id === teamId);
              return (
                <section key={teamId ?? "workspace"}>
                  <div className="mb-2 flex items-center justify-between">
                    ...
                    <Button
                      ...
                      onClick={() =>
                        setSelected((current) => {
                          ...
                          const allSelected = teamIssues.every((issue) => next.has(issue.id));
                    ...
                    >
                      Select {teamIssues.every((issue) => selected.has(issue.id)) ? "none" : "all"}
                    </Button>
                  </div>
                  <div className="overflow-hidden rounded-xl border bg-card">
                    {teamIssues.map((issue) => {
                      const creator = actors.find((actor) => actor.id === issue.creatorActorId);
                      const isPending = pending.has(issue.id);
                      return (
                        <div
                        ...
                        >
                          ...
                          <div className="min-w-0 flex-1">
                            ...
                            <div className="mt-1 flex items-center gap-2 text-[11px] text-muted-foreground">
                              ...
                              <span>·</span>
                              <span>{formatRelativeTimeLabel(issue.createdAt)}</span>
                            </div>
                          </div>
                          <Button
                            size="xs"
                            variant="ghost"
                            disabled={isPending}
                            onClick={() => void triage([issue], "decline")}
                          >
                            Decline
                          </Button>
                          <Button
                            size="xs"
                            disabled={isPending}
                            onClick={() => void triage([issue], "accept")}
                          >
                            Accept
                          </Button>
                        </div>
