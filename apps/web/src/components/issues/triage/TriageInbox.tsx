...
export function TriageInbox() {
  ...
  return <div className="flex min-h-0 flex-1 flex-col overflow-hidden bg-background">
    ...
    {triageIssues.length === 0 ? <Empty><EmptyHeader><EmptyMedia variant="icon"><InboxIcon /></EmptyMedia><EmptyTitle>Inbox zero</EmptyTitle><EmptyDescription>There are no issues waiting for triage.</EmptyDescription></EmptyHeader></Empty> : <div className="min-h-0 flex-1 overflow-y-auto p-5"><div className="mx-auto max-w-5xl space-y-6">{grouped.map(([teamId, teamIssues]) => { const team = teams.find((candidate) => candidate.id === teamId); return <section key={teamId ?? "workspace"}><div className="mb-2 flex items-center justify-between"><h2 className="text-xs font-semibold uppercase tracking-wide text-muted-foreground">{team?.icon ? `${team.icon} ` : ""}{team?.name ?? "Workspace"}</h2><Button size="xs" variant="ghost" onClick={() => setSelected((current) => { const next = new Set(current); const allSelected = teamIssues.every((issue) => next.has(issue.id)); for (const issue of teamIssues) { if (allSelected) next.delete(issue.id); else next.add(issue.id); } return next; })}>Select {teamIssues.every((issue) => selected.has(issue.id)) ? "none" : "all"}</Button></div><div className="overflow-hidden rounded-xl border bg-card">{teamIssues.map((issue) => { const creator = actors.find((actor) => actor.id === issue.creatorActorId); const isPending = pending.has(issue.id); return <div className="flex items-center gap-3 border-t border-border/60 px-4 py-3 first:border-t-0" key={issue.id}><Checkbox checked={selected.has(issue.id)} onCheckedChange={(checked) => setSelected((current) => { const next = new Set(current); if (checked) next.add(issue.id); else next.delete(issue.id); return next; })} aria-label={`Select ${issue.identifier}`} /><div className="min-w-0 flex-1"><div className="flex items-center gap-2"><span className="font-mono text-[11px] text-muted-foreground">{issue.identifier}</span><span className="truncate text-sm font-medium">{issue.title}</span></div><div className="mt-1 flex items-center gap-2 text-[11px] text-muted-foreground"><span>{creator?.displayName ?? "Unknown creator"}</span>{creator?.kind === "agent" ? <Badge variant="secondary"><BotIcon />robot</Badge> : null}<span>·</span><span>{formatRelativeTimeLabel(issue.createdAt)}</span></div></div><Button size="xs" variant="ghost" disabled={isPending} onClick={() => void triage([issue], "decline")}>Decline</Button><Button size="xs" disabled={isPending} onClick={() => void triage([issue], "accept")}>Accept</Button></div>; })}</div></section>; })}</div></div>}
  </div>;
}
