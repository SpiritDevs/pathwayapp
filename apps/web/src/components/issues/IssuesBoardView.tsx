...
function createInputForGroup(
  ...
  switch (groupBy) {
    case "state":
      return { ...base, stateId: value as IssueStateId };
    case "assignee":
      return { ...base, assigneeActorId: value as IssueActorId | null };
    case "priority":
      return { ...base, priority: value as IssuePriority };
    case "label":
      return { ...base, labelIds: value === null ? [] : [value as IssueLabelId] };
    case "cycle":
      return { ...base, cycleId: value as IssueCycleId | null };
    case "epic":
      return { ...base, epicId: value as IssueEpicId | null };
    case "team":
      return { ...base, teamId: value as IssueTeamId | null };
    case "none":
      return base;
  }
}
...
export function IssuesBoardView({
  ...
  const groups = useMemo(
    () =>
      groupIssues(issues, display.groupBy === "none" ? "state" : display.groupBy, display, lookup),
    ...
    () =>
      laneBy === "none"
        ...
        : groupIssues(
            issues,
            laneBy as GroupBy,
            { ...display, groupBy: laneBy as GroupBy },
            lookup,
          ),
    [display, issues, laneBy, lookup],
  );
  ...
  const labelsFor = (issue: EnvironmentIssue) =>
    lookup.labels.filter((label) => issue.labelIds.includes(label.id));
  const assigneeFor = (issue: EnvironmentIssue) =>
    lookup.actors.find((actor) => actor.id === issue.assigneeActorId) ?? null;
  const progressFor = (issue: EnvironmentIssue) => {
    ...
    const completedStateIds = new Set(
      lookup.states.filter((state) => state.category === "completed").map((state) => state.id),
    );
    return {
      total: children.length,
      completed: children.filter((child) => completedStateIds.has(child.stateId)).length,
    };
  };
          ...
          renderItem={({ item }) => (
            <div className="pb-2">
              <IssueCard
                ...
                subIssueProgress={progressFor(item)}
  ...
  const submitQuickCreate = async (group: IssueGroup) => {
    ...
    const targetState =
      groupBy === "state" ? lookup.states.find((state) => state.id === group.value) : null;
    const targetTeamId =
      groupBy === "team" ? (group.value as IssueTeamId | null) : (targetState?.teamId ?? null);
    const fallbackState =
      lookup.states.find(
        (state) => state.category === "unstarted" && state.teamId === targetTeamId,
      ) ??
      lookup.states.find((state) => state.category === "unstarted" && state.teamId === null) ??
      lookup.states[0];
    if (!fallbackState) return;
    const input = createInputForGroup(title, groupBy, group.value, fallbackState.id, targetTeamId);
  ...
  const handleDragEnd = (event: DragEndEvent) => {
    ...
    const group = groups.find(
      (candidate) =>
        candidate.id === overId || candidate.issues.some((issue) => issue.id === event.over?.id),
    );
    if (!group) return;
    ...
    const target = patchForGroup(
      display.groupBy === "none" ? "state" : display.groupBy,
      group.value,
    );
    void (async () => {
      if (target.teamId !== undefined) {
        await moveTeam({
          ...
          input: { issueId: active.id, teamId: target.teamId },
        });
      }
      await updateIssue({
        environmentId: active.environmentId,
        input: { issueId: active.id, patch: { ...target.patch, orderKey } },
      });
    })();
  };
  ...
  const columnHeader = (group: IssueGroup) => (
    <header className="flex h-10 items-center gap-2 border-b px-2.5 text-xs font-medium">
      ...
      <Button
        ...
        onClick={() => setQuickCreateColumn(group.id)}
      >
        <PlusIcon />
      </Button>
      <Button
    ...
    >
          ...
          {groups.map((group) =>
            collapsed.includes(group.id) ? (
              <button
                key={group.id}
                type="button"
                className="flex w-10 shrink-0 flex-col items-center gap-2 rounded-lg border bg-background py-2 text-xs"
                onClick={() => toggleCollapsed(group.id)}
              >
              ...
              <section
                key={group.id}
                className="flex h-full w-72 shrink-0 flex-col overflow-hidden rounded-lg border bg-background"
              >
                {columnHeader(group)}
                {quickCreateColumn === group.id ? (
                  <div className="border-b p-2">
                    <Input
                      ...
                      onBlur={() => {
                        if (!quickCreateTitle.trim()) setQuickCreateColumn(null);
                      }}
                      onKeyDown={(event) => {
                        if (event.key === "Enter") {
                          event.preventDefault();
                          void submitQuickCreate(group);
                        }
                ...
                {laneBy === "none" ? (
                  renderCards(group.issues, group.id)
                ) : (
                  <div className="min-h-0 flex-1 overflow-y-auto">
                    {lanes.map((lane) => {
                      ...
                      return (
                        <section key={lane.id} className="border-b last:border-b-0">
                          <div className="sticky top-0 z-10 bg-muted/92 px-2.5 py-1.5 text-[11px] font-medium text-muted-foreground">
                            {lane.label} · {cellIssues.length}
                          </div>
                          <div className="h-72">
                            {renderCards(cellIssues, `${group.id}|${lane.id}`)}
                          </div>
                        </section>
                      );
                    })}
      ...
      <DragOverlay dropAnimation={null}>
        {activeIssue ? (
          <IssueCard
            issue={activeIssue}
            labels={labelsFor(activeIssue)}
            assignee={assigneeFor(activeIssue)}
            subIssueProgress={progressFor(activeIssue)}
            overlay
          />
        ) : null}
      </DragOverlay>
