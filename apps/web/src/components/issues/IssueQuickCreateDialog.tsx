...
export const openIssueQuickCreate = () =>
  window.dispatchEvent(new CustomEvent(OPEN_QUICK_CREATE_EVENT));
...
export function IssueQuickCreateDialog() {
  ...
  const submit = async () => {
    ...
    const result = await createIssue({
      environmentId,
      input: { title: title.trim(), teamId: teamId === "workspace" ? null : teamId, priority },
    });
    setSubmitting(false);
    ...
    <Dialog open={open} onOpenChange={setOpen}>
      <DialogPopup>
        <DialogHeader>
          ...
          <DialogDescription>
            Capture the work now; details can be added from the issue page.
          </DialogDescription>
        </DialogHeader>
        <DialogPanel className="space-y-4">
          ...
          <div className="grid grid-cols-2 gap-3">
            <Select
              ...
              onValueChange={(value) => {
                if (value !== null) setTeamId(value as IssueTeamId | "workspace");
              }}
            >
              <SelectTrigger>
        ...
        <DialogFooter>
          ...
          <Button
            disabled={!title.trim() || !meta.online || submitting}
            onClick={() => void submit()}
          >
