...
function ViewItem({ view, pinned }: { view: IssueSavedView; pinned: boolean }) {
  ...
  return (
    <SidebarMenuItem>
      <SidebarMenuButton
        size="sm"
        isActive={selectedViewId === view.id}
      ...
      <Menu>
        ...
        <MenuPopup align="start" side="right">
          <MenuItem onClick={() => togglePinnedView(view.id)}>
            <PinIcon />
            {pinned ? "Unpin" : "Pin"}
          </MenuItem>
          <MenuItem
            onClick={() => {
              ...
              if (name)
                void updateView({ environmentId, input: { viewId: view.id, patch: { name } } });
            }}
          >
            Rename
          </MenuItem>
          <MenuItem
            onClick={() =>
              void updateView({
                environmentId,
                input: { viewId: view.id, patch: { position: Math.max(0, view.position - 1) } },
              })
            }
          >
            <ChevronUpIcon />
          ...
          <MenuItem
            onClick={() =>
              void updateView({
                environmentId,
                input: { viewId: view.id, patch: { position: view.position + 1 } },
              })
            }
          >
            <ChevronDownIcon />
          ...
          <MenuItem
            ...
            onClick={() => {
              if (window.confirm(`Delete “${view.name}”?`))
                void deleteView({ environmentId, input: { viewId: view.id } });
            }}
          >
            <Trash2Icon />
            Delete
...
export function SavedViewsRail() {
  ...
  const save = async () => {
    ...
    if (selected && dirty) {
      await updateView({
        environmentId,
        input: { viewId: selected.id, patch: { filters, display } },
      });
    ...
    await createView({
      ...
      input: {
        ...
        filters,
        display,
        position: sortedViews.length,
      },
    });
  };

  const renderSection = (label: string, items: ReadonlyArray<IssueSavedView>) =>
    items.length ? (
      <SidebarGroup className="py-1">
        <SidebarGroupLabel>{label}</SidebarGroupLabel>
        <SidebarMenu>
          {items.map((view) => (
            <ViewItem key={view.id} view={view} pinned={pinnedViewIds.includes(view.id)} />
          ))}
        </SidebarMenu>
      </SidebarGroup>
    ) : null;
    ...
    <>
      <SidebarHeader className="h-[var(--workspace-topbar-height)] shrink-0 flex-row items-center border-b px-3 py-0">
        ...
        <Button
          ...
          aria-label={selected && dirty ? "Update view" : "Save view"}
        >
      ...
      <SidebarContent className="gap-0 py-2">
        <SidebarGroup className="py-1">
          <SidebarMenu>
            <SidebarMenuItem>
              <SidebarMenuButton
                size="sm"
                isActive={selectedViewId === null}
                className="text-[13px]"
                onClick={() => useIssuesUiStateStore.getState().selectView(null)}
              >
                <BookmarkIcon className="size-3.5" />
        ...
        {!sortedViews.length ? (
          <p className="px-4 py-6 text-center text-xs text-muted-foreground">
            Save the current filters to create your first view.
          </p>
        ) : null}
      ...
      <SidebarFooter className="border-t">
        <Button
          ...
          className="w-full"
          onClick={() => void save()}
          disabled={!meta.online || (selected !== null && !dirty)}
        >
          <SaveIcon />
