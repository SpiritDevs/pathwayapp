...
function DroppableGroup({ id, children }: { id: string; children: ReactNode }) {
  ...
  return <section ref={setNodeRef} className={`min-w-[720px] ${isOver ? "bg-primary/4" : ""}`}>{children}</section>;
}
...
export function IssuesListView({
  ...
  const groups = useMemo(() => groupIssues(issues, display.groupBy, display, lookup), [display, issues, lookup]);
  ...
  const selectedIssues = useMemo(() => {
    ...
    return explicit.length ? explicit : orderedIssues[focusedIndex] ? [orderedIssues[focusedIndex]!] : [];
  }, [focusedIndex, orderedIssues, selectedIds]);

  useEffect(() => {
    const issue = orderedIssues[focusedIndex];
    if (!issue) return;
    document.querySelector<HTMLElement>(`[data-issue-id="${CSS.escape(issue.id)}"]`)?.scrollIntoView({ block: "nearest" });
  }, [focusedIndex, orderedIssues]);

  const select = useCallback((ref: ScopedIssueRef, options: { range: boolean; additive: boolean }) => {
    ...
    setSelectedIds((current) => {
      if (options.range && selectionAnchor) {
        ...
        if (anchorIndex >= 0) return new Set(orderedIssues.slice(Math.min(anchorIndex, index), Math.max(anchorIndex, index) + 1).map((issue) => issue.id));
      }
      if (options.additive) {
  ...
  useEffect(() => {
    const onKeyDown = (event: KeyboardEvent) => {
      ...
      if (key === "j" || key === "arrowdown") setFocusedIndex((index) => Math.min(orderedIssues.length - 1, index + 1));
  ...
  const patchSelected = async (patch: Parameters<typeof updateIssue>[0]["input"]["patch"]) => {
    if (readOnly) return;
    await Promise.all(selectedIssues.map((issue) => updateIssue({ environmentId: issue.environmentId, input: { issueId: issue.id, patch } })));
  ...
  const handleBulkProperty = (property: IssueBulkProperty, issue: EnvironmentIssue) => {
    ...
    if (property === "delete") {
      void Promise.all((selectedIds.has(issue.id) ? selectedIssues : [issue]).map((item) => deleteIssue({ environmentId: item.environmentId, input: { issueId: item.id } })));
      return;
    }
  ...
  const handleDragEnd = (event: DragEndEvent) => {
    ...
    const targetGroup = groups.find((group) => group.id === event.over?.id || group.issues.some((issue) => issue.id === event.over?.id));
    ...
    void (async () => {
      if (groupPatch.teamId !== undefined) {
        await moveTeam({ environmentId: activeIssue.environmentId, input: { issueId: activeIssue.id, teamId: groupPatch.teamId } });
      }
      await updateIssue({ environmentId: activeIssue.environmentId, input: { issueId: activeIssue.id, patch: { ...(groupPatch.patch ?? {}), orderKey } } });
    })();
  };
  ...
  return (
    <DndContext sensors={sensors} collisionDetection={closestCenter} onDragEnd={handleDragEnd}>
        ...
        {groups.map((group) => {
          ...
          return (
            <DroppableGroup key={group.id} id={group.id}>
              <button type="button" className="sticky top-0 z-10 flex h-8 w-full items-center gap-2 border-b bg-muted/92 px-3 text-left text-xs font-medium backdrop-blur" onClick={() => toggleCollapsed(group.id)}>
                ...
                <span>{group.label}</span><span className="text-muted-foreground">{group.issues.length}</span>
              </button>
              {!isCollapsed ? (
                <SortableContext items={group.issues.map((issue) => issue.id)} strategy={verticalListSortingStrategy}>
                  <LegendList<EnvironmentIssue>
                    ...
                    renderItem={({ item }) => {
                      const state = lookup.states.find((candidate) => candidate.id === item.stateId);
                      if (!state) return null;
                      return <IssueRow issue={item} state={state} labels={lookup.labels.filter((label) => item.labelIds.includes(label.id))} assignee={lookup.actors.find((actor) => actor.id === item.assigneeActorId) ?? null} selected={selectedIds.has(item.id)} focused={orderedIssues[focusedIndex]?.id === item.id} dragDisabled={readOnly || display.orderBy !== "manual"} onSelect={select} onPeek={onPeek} onBulkProperty={(property) => handleBulkProperty(property, item)} />;
                    }}
                  />
                </SortableContext>
              ) : null}
      ...
      <Popover open={bulkProperty !== null} onOpenChange={(open) => { if (!open) setBulkProperty(null); }}>
        <PopoverTrigger className="pointer-events-none fixed bottom-4 left-1/2 size-px opacity-0" aria-hidden="true" />
        <PopoverPopup side="top" align="center" className="w-64" viewportClassName="p-1!">
          <p className="px-2 py-1.5 text-xs font-medium text-muted-foreground">Update {selectedIssues.length} issue{selectedIssues.length === 1 ? "" : "s"}</p>
          {bulkProperty === "state" ? lookup.states.map((state) => <Button key={state.id} variant="ghost" size="sm" className="w-full justify-start" onClick={() => void patchSelected({ stateId: state.id })}><StateIcon state={state} />{state.name}</Button>) : null}
          {bulkProperty === "priority" ? PRIORITY_ORDER.map((priority) => <Button key={priority} variant="ghost" size="sm" className="w-full justify-start" onClick={() => void patchSelected({ priority })}><PriorityIcon priority={priority} />{PRIORITY_PRESENTATION[priority].label}</Button>) : null}
          {bulkProperty === "assignee" ? <><Button variant="ghost" size="sm" className="w-full justify-start" onClick={() => void patchSelected({ assigneeActorId: null })}>Unassigned</Button>{lookup.actors.map((actor) => <Button key={actor.id} variant="ghost" size="sm" className="w-full justify-start" onClick={() => void patchSelected({ assigneeActorId: actor.id })}>{actor.displayName}</Button>)}</> : null}
          {bulkProperty === "labels" ? lookup.labels.map((label) => <Button key={label.id} variant="ghost" size="sm" className="w-full justify-start" onClick={() => void patchSelected({ labelIds: [label.id] })}><span className="size-2 rounded-full" style={{ backgroundColor: label.color }} />{label.name}</Button>) : null}
        </PopoverPopup>
      </Popover>
    </DndContext>
  );
