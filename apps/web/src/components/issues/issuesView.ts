...
export function filterIssues(
  ...
  return issues.filter((issue) => {
    ...
    if (
      ...
      (issue.teamId === null || !filters.teamIds.includes(issue.teamId))
    )
      return false;
    if (filters.stateIds?.length && !filters.stateIds.includes(issue.stateId)) return false;
    if (
      filters.stateCategories?.length &&
      (!state || !filters.stateCategories.includes(state.category))
    )
      return false;
    if (
      filters.assigneeActorIds?.length &&
      (issue.assigneeActorId === null || !filters.assigneeActorIds.includes(issue.assigneeActorId))
    )
      return false;
    if (filters.creatorActorIds?.length && !filters.creatorActorIds.includes(issue.creatorActorId))
      return false;
    if (filters.priorities?.length && !filters.priorities.includes(issue.priority)) return false;
    if (
      filters.labelIds?.length &&
      !filters.labelIds.every((labelId) => issue.labelIds.includes(labelId))
    )
      return false;
    if (
      filters.cycleIds?.length &&
      (issue.cycleId === null || !filters.cycleIds.includes(issue.cycleId))
    )
      return false;
    if (
      filters.epicIds?.length &&
      (issue.epicId === null || !filters.epicIds.includes(issue.epicId))
    )
      return false;
    if (filters.parentIssueId !== undefined && issue.parentIssueId !== filters.parentIssueId)
      return false;
...
export function groupIssues(
  ...
  switch (groupBy) {
    ...
    case "assignee":
      definitions.push(
        ...lookup.actors.map((item, position) => ({
          value: item.id,
          label: item.displayName,
          color: item.avatarColor,
          position,
        })),
        { value: null, label: "Unassigned", position: 9999 },
      );
      break;
    case "priority":
      definitions.push(
        ...PRIORITY_ORDER.map((value, position) => ({
          value,
    ...
    case "label":
      definitions.push(
        ...lookup.labels.map((item, position) => ({
          value: item.id,
          label: item.name,
          color: item.color,
          position,
        })),
        { value: null, label: "No label", position: 9999 },
      );
      break;
    case "cycle":
      definitions.push(
        ...lookup.cycles.map((item, position) => ({
          value: item.id,
          label: item.name ?? `Cycle ${item.number}`,
          position,
        })),
        { value: null, label: "No cycle", position: 9999 },
      );
      break;
    case "epic":
      definitions.push(
        ...lookup.epics.map((item, position) => ({
          value: item.id,
          label: item.name,
          color: item.color ?? undefined,
          position,
        })),
        { value: null, label: "No epic", position: 9999 },
      );
      break;
    case "team":
      definitions.push(
        ...lookup.teams.map((item, position) => ({
          value: item.id,
          label: item.name,
          color: item.color ?? undefined,
          position,
        })),
        { value: null, label: "Workspace", position: 9999 },
      );
      break;
    case "none":
      definitions.push({ value: "all", label: "All issues", position: 0 });
      break;
  }
  return definitions
    .filter(
      (definition) =>
        buckets.has(definition.value === null ? "none" : String(definition.value)) ||
        groupBy !== "none",
    )
    ...
    .map((definition) => ({
      ...
      issues: sortedIssues(
        buckets.get(definition.value === null ? "none" : String(definition.value)) ?? [],
        display,
      ),
    }));
}
...
export function patchForGroup(
  ...
  switch (groupBy) {
    case "state":
      return { patch: { stateId: value as IssueStateId } };
    case "assignee":
      return { patch: { assigneeActorId: value as IssueActorId | null } };
    case "priority":
      return { patch: { priority: value as IssuePriority } };
    case "label":
      return { patch: { labelIds: value === null ? [] : [value as IssueLabelId] } };
    case "cycle":
      return { patch: { cycleId: value as IssueCycleId | null } };
    case "epic":
      return { patch: { epicId: value as IssueEpicId | null } };
    case "team":
      return { teamId: value as IssueTeamId | null };
    case "none":
      return {};
  }
}
