...
describe("hostedPairing", () => {
  ...
  it("reads hosted pairing host and query token parameters", () => {
    const url = new URL("https://app.pathwayos.codes/pair?host=100.64.1.2:3773&token=ABCD1234");
  ...
  it("prefers hash tokens so generated hosted links do not put credentials in search params", () => {
    vi.stubEnv("VITE_HOSTED_APP_URL", "https://preview.pathwayos.codes");
    ...
    expect(url.origin).toBe("https://preview.pathwayos.codes");
  ...
  it("builds hosted channel selection URLs through the configured router origin", () => {
    vi.stubEnv("VITE_HOSTED_APP_URL", "https://app.pathwayos.codes");
    ...
    expect(url.origin).toBe("https://app.pathwayos.codes");
    expect(url.pathname).toBe("/__pathwayos/channel");
  ...
  it("ignores incomplete hosted pairing requests", () => {
    expect(
      hasHostedPairingRequest(new URL("https://app.pathwayos.codes/pair?host=backend.example.com")),
    ).toBe(false);
    expect(
      hasHostedPairingRequest(new URL("https://app.pathwayos.codes/pair?token=ABCD1234")),
    ).toBe(false);
  });
  ...
  it("detects hosted channel aliases as static apps", () => {
    ...
    expect(isHostedStaticApp(new URL("https://nightly.app.pathwayos.codes/"))).toBe(true);

    vi.stubEnv("VITE_HTTP_URL", "https://backend.example.com");
    expect(isHostedStaticApp(new URL("https://nightly.app.pathwayos.codes/"))).toBe(false);
  });
});
