...
describe("Pierre file icons", () => {
  ...
  it("ships every custom icon referenced by the extended resolver", () => {
    const customIconNames = new Set(Object.values(PathwayOS_PIERRE_ICONS.byFileName));
    for (const iconName of customIconNames) {
      assert.include(PathwayOS_PIERRE_ICONS.spriteSheet, `id="${iconName}"`);
    }
  });
