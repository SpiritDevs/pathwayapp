...
const remoteDebuggingPort = process.env.PATHWAYOS_DESKTOP_REMOTE_DEBUGGING_PORT?.trim();
// oxlint-disable-next-line pathwayos/no-global-process-runtime -- Standalone dev script has no Effect runtime.
...
if (devProtocolClient) {
  childEnv.PATHWAYOS_DESKTOP_APP_USER_MODEL_ID = devProtocolClient.appBundleId;
  childEnv.PATHWAYOS_DESKTOP_PROTOCOL_REGISTRATION_MANAGED = "1";
}
...
function cleanupStaleDevApps() {
  ...
  NodeChildProcess.spawnSync("pkill", ["-f", "--", `--pathwayos-dev-root=${desktopDir}`], {
    stdio: "ignore",
  });
}
...
function startApp() {
  ...
  const launchArgs = devProtocolClient
    ? electronArgs
    : [...electronArgs, `--pathwayos-dev-root=${desktopDir}`, "dist-electron/main.cjs"];
