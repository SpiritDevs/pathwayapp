// This file mostly exists because we want dev mode to say "pathwayOS (Dev)" instead of "electron"
...
const devBundleIdSuffix = NodePath.basename(repoRoot)
  .toLowerCase()
  .replaceAll(/[^a-z0-9]+/g, "");
export const APP_DISPLAY_NAME = isDevelopment ? "pathwayOS (Dev)" : "pathwayOS (Alpha)";
export const APP_BUNDLE_ID = isDevelopment
  ? `com.pathwayos.pathwayos.dev.${devBundleIdSuffix || "local"}`
  : "com.pathwayos.pathwayos";
const APP_PROTOCOL_SCHEMES = isDevelopment ? ["pathwayos-dev"] : ["pathwayos"];
...
// oxlint-disable-next-line pathwayos/no-global-process-runtime -- Standalone launcher script has no Effect runtime.
...
export function makeDevelopmentLauncherScript({
  ...
  const envEntries = [
    ...
    ["PATHWAYOS_PORT", environment.PATHWAYOS_PORT],
    ["PATHWAYOS_HOME", environment.PATHWAYOS_HOME],
    ["PATHWAYOS_COMMIT_HASH", environment.PATHWAYOS_COMMIT_HASH],
    ["PATHWAYOS_OTLP_TRACES_URL", environment.PATHWAYOS_OTLP_TRACES_URL],
    ["PATHWAYOS_OTLP_EXPORT_INTERVAL_MS", environment.PATHWAYOS_OTLP_EXPORT_INTERVAL_MS],
    ["PATHWAYOS_DESKTOP_APP_USER_MODEL_ID", APP_BUNDLE_ID],
  ...
  return [
    ...
    `exec ${shellSingleQuote(electronBinaryPath)} --pathwayos-dev-root=${shellSingleQuote(desktopRoot)} ${shellSingleQuote(mainEntryPath)} "$@"`,
    "",
  ].join("\n");
}
