...
import { ConnectionCatalogDocument } from "@pathwayos/client-runtime/platform";
import { EnvironmentId, type PersistedSavedEnvironmentRecord } from "@pathwayos/contracts";
...
function makeLayer(
  ...
  const environmentLayer = DesktopEnvironment.layer({
    ...
    Layer.provide(
      Layer.mergeAll(NodeServices.layer, DesktopConfig.layerTest({ PATHWAYOS_HOME: baseDir })),
    ),
  );
...
const withStore = <A, E, R>(
  ...
  Effect.gen(function* () {
    ...
    const baseDir = yield* fileSystem.makeTempDirectoryScoped({
      prefix: "pathwayos-desktop-connection-catalog-test-",
    });
