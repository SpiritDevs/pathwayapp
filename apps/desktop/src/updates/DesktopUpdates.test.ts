...
import type { DesktopUpdateState } from "@pathwayos/contracts";
...
function makeHarness(options: UpdatesHarnessOptions = {}) {
  ...
  const environmentLayer = DesktopEnvironment.layer({
    ...
    homeDirectory: `/tmp/pathwayos-desktop-updates-home-${process.pid}`,
    platform: "darwin",
    processArch: "x64",
    appVersion: "1.2.3",
    appPath: "/repo",
    ...
    Layer.provide(
      Layer.mergeAll(
        ...
        DesktopConfig.layerTest({
          PATHWAYOS_HOME: `/tmp/pathwayos-desktop-updates-test-${process.pid}`,
          PATHWAYOS_DESKTOP_MOCK_UPDATES: "true",
          PATHWAYOS_DESKTOP_MOCK_UPDATE_SERVER_PORT: "4141",
          ...options.env,
        }),
      ),
    ),
