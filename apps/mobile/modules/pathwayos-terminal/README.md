# PathwayOS Mobile Terminal Native Module
...
fork, with PathwayOS's iOS 16 compatibility patch applied. `PathwayOSTerminalView` owns a `libghostty` surface and
...
Android currently implements the same view name (`PathwayOSTerminalSurface`) and event payloads so the
...
```bash
apps/mobile/modules/pathwayos-terminal/scripts/build-libghostty-ios16.sh
```
