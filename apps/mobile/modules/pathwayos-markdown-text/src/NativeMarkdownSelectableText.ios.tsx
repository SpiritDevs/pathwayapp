...
export function NativeMarkdownSelectableText(props: {
  ...
  // PathwayOSMarkdownText only rebuilds its attributed string during native layout. A
      ...
      {keyedRuns.map(({ key, run, text }) => {
        ...
        return (
          <MarkdownTextPrimitive
            ...
            nativeID={
              run.fileIcon
                ? `pathwayos-file:${Image.resolveAssetSource(markdownFileIconSource(run.fileIcon)).uri}`
                : run.skillName
                  ? "pathwayos-skill:sf:cube"
                  : undefined
            }
