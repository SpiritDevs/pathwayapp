...
export const createOxlintRuleHarness = (
  ...
  const run: RuleHarness["run"] = Effect.fnUntraced(function* (source: string) {
    ...
    const fixtureDir = yield* fs.makeTempDirectoryScoped({ prefix: "pathwayos-oxlint-" });
    ...
    const pluginPath = path.join(repoRoot, "oxlint-plugin-pathwayos", "index.ts");

    yield* fs.writeFileString(
      configPath,
      yield* encodeOxlintConfig({
        jsPlugins: [{ name: "pathwayos", specifier: pluginPath }],
