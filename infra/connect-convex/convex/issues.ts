...
import {
  requireEnvironmentPrincipal,
  type EnvironmentPrincipal,
} from "./environmentAuthorization.ts";
...
const issueCommand = v.union(
  ...
  v.object({
    type: v.literal("issue.startWork"),
    issueId: v.id("issues"),
    repoLogicalKey: v.optional(v.string()),
  }),
  v.object({
    type: v.literal("issue.setDelegationStatus"),
    issueId: v.id("issues"),
    status: delegationStatus,
  }),
  v.object({
    type: v.literal("comment.create"),
    issueId: v.id("issues"),
    parentCommentId: v.optional(v.id("issueComments")),
    bodyMd: v.string(),
  }),
  v.object({
    type: v.literal("comment.update"),
    commentId: v.id("issueComments"),
    bodyMd: v.string(),
  }),
  v.object({ type: v.literal("comment.delete"), commentId: v.id("issueComments") }),
  v.object({
    type: v.literal("reaction.toggle"),
    commentId: v.id("issueComments"),
    emoji: v.string(),
  }),
  v.object({
    type: v.literal("relation.create"),
    issueId: v.id("issues"),
  ...
  v.object({
    type: v.literal("threadLink.create"),
    issueId: v.id("issues"),
    threadId: v.string(),
    environmentId: v.string(),
    logicalProjectKey: v.optional(v.string()),
  }),
  v.object({
    type: v.literal("threadLink.update"),
    linkId: v.id("issueThreadLinks"),
  ...
  v.object({
    ...
    patch: v.object({
      ...
      cycleConfig: v.optional(cycleConfig),
  ...
  v.object({
    type: v.literal("state.delete"),
    stateId: v.id("issueStates"),
    migrateToStateId: v.id("issueStates"),
  }),
  v.object({
    type: v.literal("label.create"),
    teamId: v.union(v.null(), v.id("issueTeams")),
    name: v.string(),
    color: v.string(),
    description: v.optional(v.string()),
  ...
  v.object({
    type: v.literal("cycle.update"),
    cycleId: v.id("issueCycles"),
    patch: v.object({ name: optionalNullableString }),
  }),
  v.object({
    type: v.literal("epic.create"),
    name: v.string(),
    description: v.optional(v.string()),
    icon: v.optional(v.string()),
    color: v.optional(v.string()),
  ...
  v.object({
    type: v.literal("milestone.create"),
    epicId: v.id("issueEpics"),
    name: v.string(),
    targetDate: v.optional(v.string()),
    position: v.number(),
  ...
  v.object({
    type: v.literal("milestone.update"),
    milestoneId: v.id("issueMilestones"),
    patch: v.object({
      name: v.optional(v.string()),
      targetDate: optionalNullableString,
      position: v.optional(v.number()),
  ...
  v.object({
    type: v.literal("view.update"),
    viewId: v.id("issueSavedViews"),
    patch: v.object({
      name: v.optional(v.string()),
      icon: optionalNullableString,
      color: optionalNullableString,
      filters: v.optional(filterConfig),
      display: v.optional(displayConfig),
      position: v.optional(v.number()),
    }),
  }),
  v.object({ type: v.literal("view.delete"), viewId: v.id("issueSavedViews") }),
  v.object({
    type: v.literal("agent.create"),
    displayName: v.string(),
    avatarColor: v.string(),
    config: agentConfig,
  }),
  v.object({
    type: v.literal("agent.update"),
    actorId: v.id("issueActors"),
    patch: v.object({
      displayName: v.optional(v.string()),
      avatarColor: v.optional(v.string()),
      config: v.optional(agentConfig),
    }),
  }),
...
type AttributionInput =
  ...
  | {
      readonly kind: "agent";
      readonly actorId: GenericId<"issueActors">;
...
async function allocateSyncSeq(
  ctx: MutationCtx,
  principal: EnvironmentPrincipal,
  now: string,
): Promise<number> {
  ...
  await ctx.db.patch("issueMeta", meta._id, {
    nextSyncSeq: sequence + 1,
    syncSeq: sequence,
    updatedAt: now,
  });
  return sequence;
...
async function seedStates(
  ctx: MutationCtx,
  principal: EnvironmentPrincipal,
  teamId: DocumentByName<DataModel, "issueTeams">["_id"] | null,
  now: string,
): Promise<void> {
...
async function ensureSeeded(
  ...
  principal: EnvironmentPrincipal,
  now: string,
): Promise<DocumentByName<DataModel, "issueActors">> {
  let meta = await ctx.db
    .query("issueMeta")
    .withIndex("by_tenant", (query) => query.eq("tenantId", principal.tenantId))
    .unique();
  if (meta === null) {
    const metaId = await ctx.db.insert("issueMeta", {
      tenantId: principal.tenantId,
      ownerUserId: principal.ownerUserId,
  ...
  let actor = await ctx.db
    .query("issueActors")
    .withIndex("by_tenant_owner_kind", (query) =>
      query
        .eq("tenantId", principal.tenantId)
        .eq("ownerUserId", principal.ownerUserId)
        .eq("kind", "human"),
    )
    .unique();
  if (actor === null) {
    const syncSeq = await allocateSyncSeq(ctx, principal, now);
    const actorId = await ctx.db.insert("issueActors", {
      tenantId: principal.tenantId,
      ownerUserId: principal.ownerUserId,
      kind: "human",
      displayName: principal.ownerUserId,
      avatarColor: "#64748B",
      avatarUrl: null,
      syncSeq,
      createdAt: now,
...
async function actorForAttribution(
  ctx: MutationCtx,
  principal: EnvironmentPrincipal,
  attribution: AttributionInput,
  humanActor: DocumentByName<DataModel, "issueActors">,
): Promise<DocumentByName<DataModel, "issueActors">> {
  if (attribution.kind === "human") return humanActor;
  return assertTenant(
    await ctx.db.get("issueActors", attribution.actorId),
    principal,
    "ISSUES_ACTOR_NOT_FOUND",
  );
}

async function bumpIssueForDetail(
  ctx: MutationCtx,
  principal: EnvironmentPrincipal,
  issue: IssueDoc,
  now: string,
...
async function appendIssueEvent(
  ...
  principal: EnvironmentPrincipal,
  issue: IssueDoc,
  actorId: DocumentByName<DataModel, "issueActors">["_id"] | null,
  ...
  await ctx.db.insert("issueEvents", {
    ...
    actorId,
    kind: command.type,
    payload: sanitizedIssueCommandPayload(command),
    threadRef:
      attribution.kind === "agent"
        ? { environmentId: principal.environmentId, threadId: attribution.threadId }
        : null,
    syncSeq,
    createdAt: now,
    updatedAt: now,
...
async function nextIssueNumber(
  ...
  const counter = await ctx.db
    .query("issueCounters")
    .withIndex("by_tenant_scope", (query) =>
      query.eq("tenantId", principal.tenantId).eq("scopeKey", scopeKey),
    )
    .unique();
  if (counter === null) {
  ...
  await ctx.db.patch("issueCounters", counter._id, {
    next: number + 1,
    syncSeq: await allocateSyncSeq(ctx, principal, now),
    updatedAt: now,
    deletedAt: null,
...
async function statesForTeam(
  ctx: QueryCtx | MutationCtx,
  principal: EnvironmentPrincipal,
  teamId: DocumentByName<DataModel, "issueTeams">["_id"] | null,
): Promise<ReadonlyArray<DocumentByName<DataModel, "issueStates">>> {
  return (
    await ctx.db
      .query("issueStates")
      .withIndex("by_tenant_team", (query) =>
        query.eq("tenantId", principal.tenantId).eq("teamId", teamId),
      )
      .collect()
...
): Promise<DocumentByName<DataModel, "issueStates">> {
  const states = await statesForTeam(ctx, principal, teamId);
  const state =
    states
      .filter((candidate) => candidate.category === category)
      .sort((left, right) => left.position - right.position)[0] ??
    [...states].sort((left, right) => left.position - right.position)[0];
...
async function issueById(
  ctx: QueryCtx | MutationCtx,
  principal: EnvironmentPrincipal,
  issueId: DocumentByName<DataModel, "issues">["_id"],
): Promise<IssueDoc> {
...
async function validateIssueReferences(
  ...
  if (references.assigneeActorId !== null) {
    const assignee = assertTenant(
      await ctx.db.get("issueActors", references.assigneeActorId),
      principal,
      "ISSUES_ACTOR_NOT_FOUND",
    );
  ...
  for (const labelId of references.labelIds) {
    const label = assertTenant(
      await ctx.db.get("issueLabels", labelId),
      principal,
      "ISSUES_LABEL_NOT_FOUND",
    );
    if (label.deletedAt !== null || (label.teamId !== null && label.teamId !== teamId))
      throw new Error("ISSUES_LABEL_INVALID");
  }
  if (references.cycleId !== null) {
    const cycle = assertTenant(
      await ctx.db.get("issueCycles", references.cycleId),
      principal,
      "ISSUES_CYCLE_NOT_FOUND",
    );
    if (cycle.deletedAt !== null || cycle.teamId !== teamId)
      throw new Error("ISSUES_CYCLE_INVALID");
  }
  if (references.epicId !== null) {
    const epic = assertTenant(
      await ctx.db.get("issueEpics", references.epicId),
      principal,
      "ISSUES_EPIC_NOT_FOUND",
    );
    if (epic.deletedAt !== null) throw new Error("ISSUES_EPIC_NOT_FOUND");
  }
  if (references.milestoneId !== null) {
    const milestone = assertTenant(
      await ctx.db.get("issueMilestones", references.milestoneId),
      principal,
      "ISSUES_MILESTONE_NOT_FOUND",
    );
    if (
      milestone.deletedAt !== null ||
      (references.epicId !== null && milestone.epicId !== references.epicId)
    )
      throw new Error("ISSUES_MILESTONE_INVALID");
  }
...
async function ensureTeamCycles(
  ctx: MutationCtx,
  principal: EnvironmentPrincipal,
  teamId: DocumentByName<DataModel, "issueTeams">["_id"],
  nowMs: number,
  now: string,
): Promise<void> {
  const team = assertTenant(
    await ctx.db.get("issueTeams", teamId),
    principal,
    "ISSUES_TEAM_NOT_FOUND",
  );
  ...
  for (const window of windows) {
    let cycle = await ctx.db
      .query("issueCycles")
      .withIndex("by_team_number", (query) =>
        query.eq("teamId", teamId).eq("number", window.number),
      )
      .unique();
    if (cycle === null) {
      const cycleId = await ctx.db.insert("issueCycles", {
        tenantId: principal.tenantId,
        ownerUserId: principal.ownerUserId,
        teamId,
  ...
  const allCycles = (
    await ctx.db
      .query("issueCycles")
      .withIndex("by_team_number", (query) => query.eq("teamId", teamId))
      .collect()
  )
  ...
  for (const ended of endedCycles) {
    ...
    for (const issue of issues) {
      ...
      const state = assertTenant(
        await ctx.db.get("issueStates", issue.stateId),
        principal,
        "ISSUES_STATE_NOT_FOUND",
      );
...
export const executeCommand = internalMutationGeneric({
  ...
  handler: async (ctx, args) => {
    ...
    switch (args.command.type) {
      case "issue.create": {
        const teamId = args.command.teamId;
        let teamKey: string;
        if (teamId === null) {
          const meta = await ctx.db
            .query("issueMeta")
        ...
        const scopeKey = teamId === null ? "workspace" : `team:${teamId}`;
        ...
        if (selectedState.teamId !== teamId || selectedState.deletedAt !== null)
          throw new Error("ISSUES_STATE_INVALID");
        const siblings = await ctx.db
          .query("issues")
          .withIndex("by_tenant_team", (query) => query.eq("tenantId", principal.tenantId))
          .filter((query) => query.eq(query.field("teamId"), teamId))
          .collect();
        await validateIssueReferences(ctx, principal, teamId, {
        ...
        const issueId = await ctx.db.insert("issues", {
          ...
          milestoneId: args.command.milestoneId ?? null,
          parentIssueId: args.command.parentIssueId ?? null,
          orderKey: nextIssueOrderKey(
            siblings.filter((issue) => issue.deletedAt === null).map((issue) => issue.orderKey),
          ),
      ...
      case "issue.update": {
        ...
        if (args.command.patch.stateId !== undefined) {
          const state = assertTenant(
            await ctx.db.get("issueStates", args.command.patch.stateId),
            principal,
            "ISSUES_STATE_NOT_FOUND",
          );
          if (state.teamId !== issue.teamId || state.deletedAt !== null)
            throw new Error("ISSUES_STATE_INVALID");
        }
        const references = {
          assigneeActorId:
            args.command.patch.assigneeActorId === undefined
              ? issue.assigneeActorId
              : args.command.patch.assigneeActorId,
          labelIds: args.command.patch.labelIds ?? issue.labelIds,
          cycleId:
            args.command.patch.cycleId === undefined ? issue.cycleId : args.command.patch.cycleId,
          epicId:
            args.command.patch.epicId === undefined ? issue.epicId : args.command.patch.epicId,
          milestoneId:
            args.command.patch.milestoneId === undefined
              ? issue.milestoneId
              : args.command.patch.milestoneId,
          parentIssueId:
            args.command.patch.parentIssueId === undefined
              ? issue.parentIssueId
              : args.command.patch.parentIssueId,
        };
      ...
      case "issue.moveTeam": {
        ...
        if (args.command.teamId === null) {
          ...
          const team = assertTenant(
            await ctx.db.get("issueTeams", args.command.teamId),
            principal,
            "ISSUES_TEAM_NOT_FOUND",
          );
          key = team.key;
        ...
        const mappedState =
          destinationStates
            .filter((state) => state.category === oldState.category)
            .sort((left, right) => left.position - right.position)[0] ??
          (await defaultState(ctx, principal, args.command.teamId));
        const number = await nextIssueNumber(
          ctx,
          principal,
          args.command.teamId === null ? "workspace" : `team:${args.command.teamId}`,
          now,
        );
        const nextIdentifier = formatIssueIdentifier(key, number);
        await ctx.db.insert("issueAliases", {
          tenantId: principal.tenantId,
          ownerUserId: principal.ownerUserId,
          alias: issue.identifier,
          issueId: issue._id,
          syncSeq: await allocateSyncSeq(ctx, principal, now),
          createdAt: now,
          updatedAt: now,
      ...
      case "issue.restore": {
        const issue = await issueById(ctx, principal, args.command.issueId);
        await ctx.db.patch("issues", issue._id, {
          deletedAt: args.command.type === "issue.delete" ? now : null,
          syncSeq: await allocateSyncSeq(ctx, principal, now),
          updatedAt: now,
        });
        eventIssue = issue;
      ...
      case "issue.purge": {
        ...
        const comments = await ctx.db
          .query("issueComments")
          .withIndex("by_issue", (query) => query.eq("issueId", issue._id))
          .collect();
        for (const comment of comments) {
          const reactions = await ctx.db
            .query("issueCommentReactions")
            .withIndex("by_comment_actor_emoji", (query) => query.eq("commentId", comment._id))
            .collect();
          for (const reaction of reactions)
            await ctx.db.delete("issueCommentReactions", reaction._id);
        ...
        const reverseRelations = await ctx.db
          .query("issueRelations")
          .withIndex("by_related_issue", (query) => query.eq("relatedIssueId", issue._id))
          .collect();
        ...
        await ctx.db.insert("issueEvents", {
          ...
          actorId: actor._id,
          kind: "issue.purged",
          payload: { table: "issues", id: issue._id, purged: true },
          threadRef: null,
          syncSeq,
          createdAt: now,
          updatedAt: now,
      ...
      case "issue.setDelegationStatus": {
        const issue = await issueById(ctx, principal, args.command.issueId);
        await ctx.db.patch("issues", issue._id, {
          delegationStatus: args.command.status,
          syncSeq: await allocateSyncSeq(ctx, principal, now),
          updatedAt: now,
        });
        eventIssue = issue;
      ...
      case "comment.create": {
        ...
        if (args.command.parentCommentId !== undefined) {
          const parent = assertTenant(
            await ctx.db.get("issueComments", args.command.parentCommentId),
            principal,
            "ISSUES_COMMENT_NOT_FOUND",
          );
          if (parent.issueId !== issue._id) throw new Error("ISSUES_COMMENT_INVALID");
        }
        createdId = await ctx.db.insert("issueComments", {
          tenantId: principal.tenantId,
          ownerUserId: principal.ownerUserId,
          issueId: issue._id,
          parentCommentId: args.command.parentCommentId ?? null,
          authorActorId: actor._id,
          bodyMd: args.command.bodyMd,
          editedAt: null,
          syncSeq: await allocateSyncSeq(ctx, principal, now),
          createdAt: now,
          updatedAt: now,
      ...
      case "comment.update":
      case "comment.delete": {
        const comment = assertTenant(
          await ctx.db.get("issueComments", args.command.commentId),
          principal,
          "ISSUES_COMMENT_NOT_FOUND",
        );
        const issue = await issueById(ctx, principal, comment.issueId);
        if (args.command.type === "comment.update")
          await ctx.db.patch("issueComments", comment._id, {
            bodyMd: args.command.bodyMd,
            editedAt: now,
            syncSeq: await allocateSyncSeq(ctx, principal, now),
            updatedAt: now,
      ...
      case "reaction.toggle": {
        ...
        const existing = await ctx.db
          ...
          .filter((query) =>
            query.and(
              query.eq(query.field("actorId"), actor._id),
              query.eq(query.field("emoji"), emoji),
            ),
          )
          .unique();
        if (existing === null)
          createdId = await ctx.db.insert("issueCommentReactions", {
            tenantId: principal.tenantId,
            ownerUserId: principal.ownerUserId,
          ...
          await ctx.db.patch("issueCommentReactions", existing._id, {
            deletedAt: existing.deletedAt === null ? now : null,
            syncSeq: await allocateSyncSeq(ctx, principal, now),
            updatedAt: now,
          });
        eventIssue = issue;
      ...
      case "relation.create": {
        ...
        createdId = await ctx.db.insert("issueRelations", {
          tenantId: principal.tenantId,
          ownerUserId: principal.ownerUserId,
          issueId: issue._id,
          relationType: args.command.relationType,
          relatedIssueId: args.command.relatedIssueId,
          syncSeq: await allocateSyncSeq(ctx, principal, now),
          createdAt: now,
          updatedAt: now,
          deletedAt: null,
      ...
      case "relation.delete": {
        const relation = assertTenant(
          await ctx.db.get("issueRelations", args.command.relationId),
          principal,
          "ISSUES_RELATION_NOT_FOUND",
        );
        const issue = await issueById(ctx, principal, relation.issueId);
        await ctx.db.patch("issueRelations", relation._id, {
          deletedAt: now,
          syncSeq: await allocateSyncSeq(ctx, principal, now),
          updatedAt: now,
        });
      ...
      case "threadLink.create": {
        const issue = await issueById(ctx, principal, args.command.issueId);
        createdId = await ctx.db.insert("issueThreadLinks", {
          tenantId: principal.tenantId,
          ownerUserId: principal.ownerUserId,
          issueId: issue._id,
          environmentId: args.command.environmentId,
          threadId: args.command.threadId,
          logicalProjectKey: args.command.logicalProjectKey ?? null,
          status: "linked",
      ...
      case "threadLink.delete": {
        const link = assertTenant(
          await ctx.db.get("issueThreadLinks", args.command.linkId),
          principal,
          "ISSUES_THREAD_LINK_NOT_FOUND",
        );
        const issue = await issueById(ctx, principal, link.issueId);
        if (args.command.type === "threadLink.update")
          await ctx.db.patch("issueThreadLinks", link._id, {
      ...
      case "team.create": {
        ...
        const conflicts = await ctx.db
          .query("issueTeams")
          .withIndex("by_tenant_key", (query) => query.eq("tenantId", principal.tenantId))
          .filter((query) => query.eq(query.field("key"), key))
          .collect();
        if (conflicts.some((conflict) => conflict.deletedAt === null))
          throw new Error("ISSUES_TEAM_KEY_CONFLICT");
        const teamId = await ctx.db.insert("issueTeams", {
          tenantId: principal.tenantId,
          ownerUserId: principal.ownerUserId,
          name: args.command.name,
          description: args.command.description ?? null,
          icon: args.command.icon ?? null,
          color: args.command.color ?? null,
          key,
          cycleConfig: DEFAULT_CYCLE_CONFIG,
          estimateScale: "disabled",
          repoLinks: [],
      ...
      case "team.update": {
        ...
        const patch =
          args.command.patch.key === undefined
            ? args.command.patch
            : { ...args.command.patch, key: normalizeIssueKey(args.command.patch.key) };
        if (patch.key !== undefined) {
          ...
          const conflicts = await ctx.db
            ...
            .collect();
          if (
            conflicts.some((conflict) => conflict._id !== team._id && conflict.deletedAt === null)
          )
            throw new Error("ISSUES_TEAM_KEY_CONFLICT");
        }
        await ctx.db.patch("issueTeams", team._id, {
          ...patch,
          syncSeq: await allocateSyncSeq(ctx, principal, now),
          updatedAt: now,
        });
      ...
      case "team.memberAdd": {
        const actorId = args.command.actorId;
        const team = assertTenant(
          await ctx.db.get("issueTeams", args.command.teamId),
          principal,
          "ISSUES_TEAM_NOT_FOUND",
        );
        assertTenant(await ctx.db.get("issueActors", actorId), principal, "ISSUES_ACTOR_NOT_FOUND");
        const existing = await ctx.db
          .query("issueTeamMemberships")
          .withIndex("by_team_actor", (query) => query.eq("teamId", team._id))
          .filter((query) => query.eq(query.field("actorId"), actorId))
          .unique();
        if (existing === null)
          createdId = await ctx.db.insert("issueTeamMemberships", {
            tenantId: principal.tenantId,
            ownerUserId: principal.ownerUserId,
            teamId: team._id,
            actorId,
          ...
          await ctx.db.patch("issueTeamMemberships", existing._id, {
            deletedAt: null,
            syncSeq: await allocateSyncSeq(ctx, principal, now),
            updatedAt: now,
          });
      ...
      case "team.memberRemove": {
        const membership = assertTenant(
          await ctx.db.get("issueTeamMemberships", args.command.membershipId),
          principal,
          "ISSUES_MEMBERSHIP_NOT_FOUND",
        );
        await ctx.db.patch("issueTeamMemberships", membership._id, {
          deletedAt: now,
          syncSeq: await allocateSyncSeq(ctx, principal, now),
          updatedAt: now,
        });
      ...
      case "state.create": {
        ...
        createdId = await ctx.db.insert("issueStates", {
          ...
          teamId: args.command.teamId,
          name: args.command.name,
          color: args.command.color,
          category: args.command.category,
      ...
      case "state.update": {
        ...
        await ctx.db.patch("issueStates", state._id, {
          ...args.command.patch,
          syncSeq: await allocateSyncSeq(ctx, principal, now),
          updatedAt: now,
        });
      ...
      case "state.delete": {
        ...
        const destination = assertTenant(
          await ctx.db.get("issueStates", args.command.migrateToStateId),
          principal,
          "ISSUES_STATE_NOT_FOUND",
        );
        if (
          destination.category !== state.category ||
          destination.teamId !== state.teamId ||
          destination.deletedAt !== null ||
          destination._id === state._id
        )
          throw new Error("ISSUES_STATE_MIGRATION_INVALID");
        const issues = await ctx.db
          .query("issues")
          .withIndex("by_tenant_team", (query) => query.eq("tenantId", principal.tenantId))
          .filter((query) => query.eq(query.field("teamId"), state.teamId))
          .collect();
        for (const issue of issues)
          if (issue.stateId === state._id)
            await ctx.db.patch("issues", issue._id, {
              stateId: destination._id,
              syncSeq: await allocateSyncSeq(ctx, principal, now),
              updatedAt: now,
            });
        await ctx.db.patch("issueStates", state._id, {
      ...
      case "label.create": {
        ...
        createdId = await ctx.db.insert("issueLabels", {
          tenantId: principal.tenantId,
          ownerUserId: principal.ownerUserId,
          teamId: args.command.teamId,
          name: args.command.name,
      ...
      case "label.delete": {
        const label = assertTenant(
          await ctx.db.get("issueLabels", args.command.labelId),
          principal,
          "ISSUES_LABEL_NOT_FOUND",
        );
        if (args.command.type === "label.update")
          await ctx.db.patch("issueLabels", label._id, {
            ...args.command.patch,
            syncSeq: await allocateSyncSeq(ctx, principal, now),
            updatedAt: now,
          });
      ...
      case "cycle.update": {
        const cycle = assertTenant(
          await ctx.db.get("issueCycles", args.command.cycleId),
          principal,
          "ISSUES_CYCLE_NOT_FOUND",
        );
        await ctx.db.patch("issueCycles", cycle._id, {
          ...args.command.patch,
          syncSeq: await allocateSyncSeq(ctx, principal, now),
          updatedAt: now,
        });
      ...
      case "epic.create": {
        createdId = await ctx.db.insert("issueEpics", {
          ...
          icon: args.command.icon ?? null,
          color: args.command.color ?? null,
          status: args.command.status ?? "backlog",
          startDate: args.command.startDate ?? null,
          targetDate: args.command.targetDate ?? null,
          syncSeq: await allocateSyncSeq(ctx, principal, now),
          createdAt: now,
          updatedAt: now,
          deletedAt: null,
      ...
      case "epic.delete": {
        const epic = assertTenant(
          await ctx.db.get("issueEpics", args.command.epicId),
          principal,
          "ISSUES_EPIC_NOT_FOUND",
        );
        if (args.command.type === "epic.update")
          await ctx.db.patch("issueEpics", epic._id, {
            ...args.command.patch,
            syncSeq: await allocateSyncSeq(ctx, principal, now),
            updatedAt: now,
          });
      ...
      case "milestone.create": {
        ...
        createdId = await ctx.db.insert("issueMilestones", {
          tenantId: principal.tenantId,
          ownerUserId: principal.ownerUserId,
          epicId: args.command.epicId,
          name: args.command.name,
          targetDate: args.command.targetDate ?? null,
          position: args.command.position,
          completedAt: null,
      ...
      case "milestone.delete": {
        const milestone = assertTenant(
          await ctx.db.get("issueMilestones", args.command.milestoneId),
          principal,
          "ISSUES_MILESTONE_NOT_FOUND",
        );
        if (args.command.type === "milestone.update")
          await ctx.db.patch("issueMilestones", milestone._id, {
            ...args.command.patch,
            syncSeq: await allocateSyncSeq(ctx, principal, now),
            updatedAt: now,
          });
      ...
      case "view.create": {
        ...
        createdId = await ctx.db.insert("issueSavedViews", {
          tenantId: principal.tenantId,
          ownerUserId: principal.ownerUserId,
          ...
          icon: args.command.icon ?? null,
          color: args.command.color ?? null,
          filters: args.command.filters,
          display: args.command.display,
          position: args.command.position,
          syncSeq: await allocateSyncSeq(ctx, principal, now),
          createdAt: now,
          updatedAt: now,
      ...
      case "view.update":
      case "view.delete": {
        const view = assertTenant(
          await ctx.db.get("issueSavedViews", args.command.viewId),
          principal,
          "ISSUES_VIEW_NOT_FOUND",
        );
        if (args.command.type === "view.update")
          await ctx.db.patch("issueSavedViews", view._id, {
            ...args.command.patch,
            syncSeq: await allocateSyncSeq(ctx, principal, now),
            updatedAt: now,
          });
      ...
      case "agent.create": {
        createdId = await ctx.db.insert("issueActors", {
          tenantId: principal.tenantId,
          ownerUserId: principal.ownerUserId,
          kind: "agent",
          displayName: args.command.displayName,
          avatarColor: args.command.avatarColor,
          avatarUrl: null,
          syncSeq: await allocateSyncSeq(ctx, principal, now),
          createdAt: now,
          updatedAt: now,
      ...
      case "agent.update":
      case "agent.delete": {
        const target = assertTenant(
          await ctx.db.get("issueActors", args.command.actorId),
          principal,
          "ISSUES_ACTOR_NOT_FOUND",
        );
        ...
        if (args.command.type === "agent.update") {
          ...
          await ctx.db.patch("issueActors", target._id, {
            ...actorPatch,
            syncSeq: await allocateSyncSeq(ctx, principal, now),
            updatedAt: now,
          });
      ...
      case "workspace.update": {
        ...
        if (meta === null) throw new Error("ISSUES_META_NOT_FOUND");
        await ctx.db.patch("issueMeta", meta._id, {
          workspaceKey: normalizeIssueKey(args.command.workspaceKey),
          syncSeq: await allocateSyncSeq(ctx, principal, now),
          updatedAt: now,
        });
        break;
...
function mirrorTeam(row: DocumentByName<DataModel, "issueTeams">) {
  return {
    ...
    color: row.color,
    key: row.key,
    cycleConfig: row.cycleConfig,
    estimateScale: row.estimateScale,
    repoLinks: row.repoLinks,
    defaultRepoLogicalKey: row.defaultRepoLogicalKey,
    ...timestamps(row),
  };
}
...
export const mirrorDelta = internalQueryGeneric({
  ...
  handler: async (ctx, args) => {
    ...
    if (!Number.isSafeInteger(args.sinceSeq) || args.sinceSeq < 0)
      throw new Error("ISSUES_SYNC_SEQUENCE_INVALID");
    const limit = args.limit ?? 500;
    if (!Number.isSafeInteger(limit) || limit < 1 || limit > 2_000)
      throw new Error("ISSUES_SYNC_LIMIT_INVALID");
    const take = limit + 1;
    ...
    const [
      ...
      ctx.db
        .query("issueTeams")
        .withIndex("by_tenant_sync", (query) => query.eq("tenantId", tenantId))
        .filter((query) => query.gt(query.field("syncSeq"), sinceSeq))
        .order("asc")
        .take(take),
      ctx.db
        .query("issueTeamMemberships")
      ...
      ctx.db
        ...
        .withIndex("by_tenant_sync", (query) => query.eq("tenantId", tenantId))
        .filter((query) => query.gt(query.field("syncSeq"), sinceSeq))
        .filter((query) => query.eq(query.field("kind"), "issue.purged"))
        .order("asc")
        .take(take),
    ]);
    ...
    return {
      ...
      nextSeq,
      hasMore,
      workspaceKey: meta?.workspaceKey ?? DEFAULT_WORKSPACE_KEY,
      viewerUserId: principal.ownerUserId,
    };
  },
});
...
export const issueDetail = internalQueryGeneric({
  ...
  handler: async (ctx, args) => {
    ...
    const events = await ctx.db
      .query("issueEvents")
      .withIndex("by_issue_created", (query) => query.eq("issueId", issue._id))
      .order("desc")
      .take(200);
    return {
      issueId: issue._id,
      descriptionMd: issue.descriptionMd,
      comments: comments.map(detailComment),
      reactions: reactions.map(detailReaction),
      events: events.map(detailEvent),
    };
  },
});
