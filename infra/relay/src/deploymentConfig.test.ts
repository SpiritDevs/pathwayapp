...
describe("relayResourceNameForStage", () => {
  it("isolates production and personal stages", () => {
    expect(relayResourceNameForStage("pathwayos-relay-traces", "prod")).toBe(
      "pathwayos-relay-traces-prod",
    );
    expect(relayResourceNameForStage("pathwayos-relay-traces", "dev_julius")).toBe(
      "pathwayos-relay-traces-dev-julius",
    );
  });
});
...
describe("managed endpoint names", () => {
  it("uses the stage slug and a stable stage-scoped digest suffix", () => {
    ...
    expect(managedEndpointHostname("prod", "pathwayosrelay.com", hash)).toBe(
      "prod-abcdef0123456789.pathwayosrelay.com",
    );
    expect(managedEndpointTunnelName("dev_julius", hash)).toBe(
      "pathwayosrelay-managedendpoint-dev-julius-abcdef0123456789",
    );
  });
