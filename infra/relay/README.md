# pathwayOS Connect Relay

> [!WARNING]
> pathwayOS Connect is currently in private beta. Join the waitlist in the app under Settings > pathwayOS Connect.

The relay is the hosted control plane for pathwayOS Connect. It helps clients discover and connect to
...
The relay is intentionally not in the hot path for normal pathwayOS traffic. After a client connects,
regular API and WebSocket traffic goes directly between that client and the selected environment.
See the [pathwayOS Connect architecture overview](../../docs/cloud/pathwayos-connect-auth-flow.html) for the larger system
design.
...
- Linking pathwayOS environments to a cloud account.
...
```sh
vp run --filter pathwayos-relay deploy
```
...
```sh
vp run --filter pathwayos-relay deploy -- --stage prod
vp run --filter pathwayos-relay deploy -- --env-file .env.local
```
...
- [pathwayOS Connect Clerk Setup](../../docs/cloud/pathwayos-connect-clerk.md) for Clerk keys, JWT templates, and waitlist
  setup.
- [Relay Observability](../../docs/relay-observability.md) for deployment tracing and diagnostics.
- [pathwayOS Connect Architecture Overview](../../docs/cloud/pathwayos-connect-auth-flow.html) for the full link,
