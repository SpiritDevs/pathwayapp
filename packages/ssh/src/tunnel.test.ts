...
import {
  ...
  buildRemotePathwayOSRunnerScript,
...
describe("ssh tunnel scripts", () => {
  it("builds the remote pathwayos runner with npx and npm fallbacks", () => {
    const script = buildRemotePathwayOSRunnerScript({ nodeEngineRange: TEST_NODE_ENGINE_RANGE });

    assert.include(script, "PathwayOS_NODE_SCRIPT_PATH=''");
    assert.include(script, 'exec pathwayos "$@"');
    assert.include(script, "exec npx --yes 'pathwayos@latest' \"$@\"");
    assert.include(script, "exec npm exec --yes 'pathwayos@latest' -- \"$@\"");
    assert.include(script, "could not install 'pathwayos@latest'");
    assert.include(script, 'prepend_path_if_dir "$HOME/.local/bin"');
    assert.include(script, `PathwayOS_NODE_ENGINE_RANGE='${TEST_NODE_ENGINE_RANGE}'`);
    ...
    assert.include(script, 'for PathwayOS_NODE_BIN in "$NVM_DIR"/versions/node/*/bin');
    assert.notInclude(script, "ensure $NVM_DIR/nvm.sh is available");
  });

  it("does not hard-code a remote node engine range", () => {
    const script = buildRemotePathwayOSRunnerScript();

    assert.include(script, "PathwayOS_NODE_ENGINE_RANGE=''");
    assert.notInclude(script, TEST_NODE_ENGINE_RANGE);
  });

  it("shell-quotes package specs in the remote pathwayos runner", () => {
    const script = buildRemotePathwayOSRunnerScript({
      packageSpec: "pathwayos@nightly; touch /tmp/pathwayos-owned",
    });

    assert.include(script, "exec npx --yes 'pathwayos@nightly; touch /tmp/pathwayos-owned' \"$@\"");
    assert.include(
      script,
      "exec npm exec --yes 'pathwayos@nightly; touch /tmp/pathwayos-owned' -- \"$@\"",
    );
    assert.notInclude(script, "exec npx --yes pathwayos@nightly; touch /tmp/pathwayos-owned");
  });

  it("builds the remote pathwayos runner with a node script override", () => {
    const script = buildRemotePathwayOSRunnerScript({
    ...
    assert.include(
      script,
      "PathwayOS_NODE_SCRIPT_PATH='/Users/julius/Development/Work/codething-mvp/apps/server/dist/bin.mjs'",
    );
    assert.include(script, 'exec node "$PathwayOS_NODE_SCRIPT_PATH" "$@"');
  });

  it("uses the remote pathwayos runner for launch and pairing scripts", () => {
    const target = {
      alias: "devbox",
    ...
    assert.include(
      buildRemoteLaunchScript({ nodeEngineRange: TEST_NODE_ENGINE_RANGE }),
      `PathwayOS_NODE_ENGINE_RANGE='${TEST_NODE_ENGINE_RANGE}'`,
    );
    assert.include(
      buildRemoteLaunchScript({ nodeEngineRange: TEST_NODE_ENGINE_RANGE }),
    ...
    assert.include(buildRemoteLaunchScript(), "Remote pathwayOS server did not become ready");
    assert.include(
      buildRemoteLaunchScript({ packageSpec: "pathwayos@nightly" }),
      "pathwayos@nightly",
    );
    assert.include(
    ...
    assert.include(
      buildRemotePairingScript(target, { packageSpec: "pathwayos@nightly" }),
      "pathwayos@nightly",
    );
    assert.include(
