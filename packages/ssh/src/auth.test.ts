  ...
  it.effect("creates askpass env for cached password prompts", () =>
    Effect.gen(function* () {
      ...
      const directory = yield* fs.makeTempDirectoryScoped({
        prefix: "pathwayos-ssh-askpass-test-",
      });
      ...
      assert.equal(env.PathwayOS_SSH_AUTH_SECRET, "super-secret");
      assert.equal(env.DISPLAY, "pathwayos");
  ...
  it.effect("builds a windows askpass launcher pair", () =>
    Effect.gen(function* () {
      const descriptor = yield* buildSshAskpassHelperDescriptor({
        directory: "C:\\temp\\pathwayos-ssh-askpass",
      }).pipe(
        Effect.provide(
      ...
      assert.equal(descriptor.launcherPath, "C:\\temp\\pathwayos-ssh-askpass\\ssh-askpass.cmd");
      assert.deepEqual(
