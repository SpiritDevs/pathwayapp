...
import {
  ...
  resolveRemotePathwayOSCliPackageSpec,
  runSshCommand,
} from "./command.ts";
...
describe("ssh command", () => {
  ...
  it.effect("resolves the remote pathwayos package spec from the desktop release channel", () =>
    Effect.sync(() => {
      assert.equal(
        resolveRemotePathwayOSCliPackageSpec({
          appVersion: "0.0.17",
      ...
      assert.equal(
        ...
        "pathwayos@0.0.17-nightly.20260415.44",
      );
      assert.equal(
        resolveRemotePathwayOSCliPackageSpec({
