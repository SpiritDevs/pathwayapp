...
import { extractJsonObject, fromLenientJson } from "@pathwayos/shared/schemaJson";
import { satisfiesSemverRange } from "@pathwayos/shared/semver";
...
export interface RemotePathwayOSRunnerOptions {
...
export interface SshEnvironmentManagerOptions {
  ...
  readonly resolveCliRunner?: Effect.Effect<RemotePathwayOSRunnerOptions>;
}
...
function sshRunnerLogFields(runner: RemotePathwayOSRunnerOptions | undefined) {
...
remote_node_satisfies_engine() {
  PathwayOS_NODE_ENGINE_RANGE=@@PathwayOS_NODE_ENGINE_RANGE@@
  if [ -z "$PathwayOS_NODE_ENGINE_RANGE" ]; then
    return 0
  fi
  node - "$PathwayOS_NODE_ENGINE_RANGE" <<'NODE'
@@PathwayOS_NODE_ENGINE_CHECK_SCRIPT@@
NODE
}
...
ensure_remote_node_path() {
  ...
  if ! command -v node >/dev/null 2>&1 && [ -d "$NVM_DIR/versions/node" ]; then
    for PathwayOS_NODE_BIN in "$NVM_DIR"/versions/node/*/bin; do
      if [ -x "$PathwayOS_NODE_BIN/node" ]; then
        PATH="$PathwayOS_NODE_BIN:$PATH"
        export PATH
      fi
    done
  fi
...
set -eu
@@PathwayOS_NODE_ENV_SCRIPT@@
ensure_remote_node_path || true
PathwayOS_NODE_SCRIPT_PATH=@@PathwayOS_NODE_SCRIPT_PATH@@
if [ -n "$PathwayOS_NODE_SCRIPT_PATH" ]; then
  ...
  exec node "$PathwayOS_NODE_SCRIPT_PATH" "$@"
fi
if command -v pathwayos >/dev/null 2>&1; then
  exec pathwayos "$@"
fi
if command -v npx >/dev/null 2>&1; then
  exec npx --yes @@PathwayOS_PACKAGE_SPEC@@ "$@"
fi
if command -v npm >/dev/null 2>&1; then
  exec npm exec --yes @@PathwayOS_PACKAGE_SPEC@@ -- "$@"
fi
printf 'Remote host is missing the pathwayos CLI and could not install @@PathwayOS_PACKAGE_SPEC@@ because node/npm/npx are unavailable on PATH. Install Node or configure a supported version manager for non-interactive shells.\\n' >&2
exit 1
`;
...
@@PathwayOS_NODE_ENV_SCRIPT@@
STATE_KEY="$1"
STATE_DIR="$HOME/.pathwayos/ssh-launch/$STATE_KEY"
DEFAULT_SERVER_HOME="$HOME/.pathwayos"
DEFAULT_RUNTIME_FILE="$DEFAULT_SERVER_HOME/userdata/server-runtime.json"
PORT_FILE="$STATE_DIR/port"
PID_FILE="$STATE_DIR/pid"
MANAGED_FILE="$STATE_DIR/managed"
LOG_FILE="$STATE_DIR/server.log"
RUNNER_FILE="$STATE_DIR/run-pathwayos.sh"
RUNNER_NEXT="$STATE_DIR/run-pathwayos.next.$$"
mkdir -p "$STATE_DIR"
cleanup_runner_next() {
  rm -f "$RUNNER_NEXT"
}
...
pick_port() {
  node - "$PORT_FILE" "@@PathwayOS_DEFAULT_REMOTE_PORT@@" "@@PathwayOS_REMOTE_PORT_SCAN_WINDOW@@" <<'NODE'
@@PathwayOS_PICK_PORT_SCRIPT@@
NODE
}
wait_ready() {
  node - "$REMOTE_PORT" "$1" "@@PathwayOS_READY_PROBE_TIMEOUT_MS@@" <<'NODE'
@@PathwayOS_WAIT_READY_SCRIPT@@
NODE
}
wait_for_pid_exit() {
  PID_TO_WAIT="$1"
...
if [ -n "$DEFAULT_REMOTE_PORT" ]; then
  ...
  if wait_ready "@@PathwayOS_REUSE_READY_TIMEOUT_MS@@"; then
...
if [ "$REMOTE_MANAGED" = "external" ]; then
  if [ -z "$REMOTE_PORT" ] || ! wait_ready "@@PathwayOS_REUSE_READY_TIMEOUT_MS@@"; then
    REMOTE_PID=""
    REMOTE_PORT=""
    REMOTE_MANAGED=""
  fi
  ...
  if [ "$RUNNER_CHANGED" -eq 1 ]; then
    ...
    REMOTE_PORT=""
    REMOTE_MANAGED=""
  elif ! wait_ready "@@PathwayOS_REUSE_READY_TIMEOUT_MS@@"; then
    kill "$REMOTE_PID" 2>/dev/null || true
    wait_for_pid_exit "$REMOTE_PID"
    REMOTE_PID=""
    REMOTE_PORT=""
...
if [ -z "$REMOTE_PORT" ]; then
  ...
  nohup env PATHWAYOS_NO_BROWSER=1 "$RUNNER_FILE" serve --host 127.0.0.1 --port "$REMOTE_PORT" --base-dir "$DEFAULT_SERVER_HOME" >>"$LOG_FILE" 2>&1 < /dev/null &
  REMOTE_PID="$!"
  printf '%s\\n' "$REMOTE_PID" >"$PID_FILE"
  printf '%s\\n' "$REMOTE_PORT" >"$PORT_FILE"
  printf 'managed\\n' >"$MANAGED_FILE"
  if ! wait_ready "@@PathwayOS_READY_TIMEOUT_MS@@"; then
    printf 'Remote pathwayOS server did not become ready on 127.0.0.1:%s.\\n' "$REMOTE_PORT" >&2
...
STATE_DIR="$HOME/.pathwayos/ssh-launch/@@PathwayOS_STATE_KEY@@"
DEFAULT_SERVER_HOME="$HOME/.pathwayos"
RUNNER_FILE="$STATE_DIR/run-pathwayos.sh"
mkdir -p "$STATE_DIR"
cat >"$RUNNER_FILE" <<'SH'
...
export function buildRemotePathwayOSRunnerScript(input?: RemotePathwayOSRunnerOptions): string {
  const packageSpec = shellSingleQuote(input?.packageSpec?.trim() || "pathwayos@latest");
  const nodeScriptPath = input?.nodeScriptPath?.trim() || "";
  return stripTrailingNewlines(
    applyScriptPlaceholders(REMOTE_RUNNER_SCRIPT, {
      PathwayOS_PACKAGE_SPEC: packageSpec,
      PathwayOS_NODE_SCRIPT_PATH: shellSingleQuote(nodeScriptPath),
      PathwayOS_NODE_ENV_SCRIPT: buildRemoteNodeEnvScript(input),
    }),
  );
}

function buildRemoteNodeEnvScript(input?: RemotePathwayOSRunnerOptions): string {
  return stripTrailingNewlines(
    applyScriptPlaceholders(REMOTE_NODE_ENV_SCRIPT, {
      PathwayOS_NODE_ENGINE_RANGE: shellSingleQuote(input?.nodeEngineRange?.trim() || ""),
      PathwayOS_NODE_ENGINE_CHECK_SCRIPT: stripTrailingNewlines(buildRemoteNodeEngineCheckScript()),
    }),
  );
}

export function buildRemoteLaunchScript(input?: RemotePathwayOSRunnerOptions): string {
  return applyScriptPlaceholders(REMOTE_LAUNCH_SCRIPT, {
    PathwayOS_NODE_ENV_SCRIPT: buildRemoteNodeEnvScript(input),
    PATHWAYOS_RUNNER_SCRIPT: stripTrailingNewlines(buildRemotePathwayOSRunnerScript(input)),
    PathwayOS_PICK_PORT_SCRIPT: stripTrailingNewlines(REMOTE_PICK_PORT_SCRIPT),
    PathwayOS_WAIT_READY_SCRIPT: stripTrailingNewlines(REMOTE_WAIT_READY_SCRIPT),
    PathwayOS_DEFAULT_REMOTE_PORT: String(DEFAULT_REMOTE_PORT),
    PathwayOS_REMOTE_PORT_SCAN_WINDOW: String(REMOTE_PORT_SCAN_WINDOW),
    PathwayOS_READY_TIMEOUT_MS: String(REMOTE_READY_TIMEOUT_MS),
    PathwayOS_REUSE_READY_TIMEOUT_MS: String(REMOTE_REUSE_READY_TIMEOUT_MS),
    PathwayOS_READY_PROBE_TIMEOUT_MS: String(SSH_READY_PROBE_TIMEOUT_MS),
  });
}

export function buildRemotePairingScript(
  target: DesktopSshEnvironmentTarget,
  input?: RemotePathwayOSRunnerOptions,
): string {
  return applyScriptPlaceholders(REMOTE_PAIRING_SCRIPT, {
    PathwayOS_STATE_KEY: remoteStateKey(target),
    PATHWAYOS_RUNNER_SCRIPT: stripTrailingNewlines(buildRemotePathwayOSRunnerScript(input)),
  });
}
...
export const launchOrReuseRemoteServer = Effect.fn("ssh/tunnel.launchOrReuseRemoteServer")(
  function* (
    ...
    runner?: RemotePathwayOSRunnerOptions,
  ): Effect.fn.Return<
...
const makeSshEnvironmentManager = Effect.fn("ssh/tunnel.SshEnvironmentManager.make")(function* (
  ...
  const createTunnelEntry = Effect.fn("ssh/tunnel.ensureTunnelEntry.create")(function* (input: {
    readonly key: string;
    readonly resolvedTarget: DesktopSshEnvironmentTarget;
    readonly runner?: RemotePathwayOSRunnerOptions;
...
>()("@pathwayos/ssh/tunnel/SshEnvironmentManager") {
