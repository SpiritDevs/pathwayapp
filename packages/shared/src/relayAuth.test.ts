...
describe("Clerk relay auth", () => {
  it("derives a custom Frontend API hostname from a Clerk publishable key", () => {
    expect(
      clerkFrontendApiHostnameFromPublishableKey(clerkPublishableKey("clerk.pathwayos.codes")),
    ).toBe("clerk.pathwayos.codes");
    expect(
      clerkFrontendApiUrlFromPublishableKey(clerkPublishableKey("clerk.pathwayos.codes")),
    ).toBe("https://clerk.pathwayos.codes");
  });
  ...
  it("reports semantic frontend API failures without inventing a cause", () => {
    ...
    const pathFrontendApi = "clerk.pathwayos.codes/path";
  ...
  it("allows standard Clerk hosts and an exact configured custom hostname", () => {
    ...
    expect(
      isAllowedClerkFrontendApiHostname("clerk.pathwayos.codes", "clerk.pathwayos.codes"),
    ).toBe(true);
    expect(isAllowedClerkFrontendApiHostname("attacker.example", "clerk.pathwayos.codes")).toBe(
      false,
    );
    expect(
      isAllowedClerkFrontendApiHostname("nested.clerk.pathwayos.codes", "clerk.pathwayos.codes"),
    ).toBe(false);
  });
});
